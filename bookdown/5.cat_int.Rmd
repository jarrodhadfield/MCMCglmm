...
```{r sigma-2}
m.sigma.2<-MCMCglmm(cbind(infected, not_infected)~virus, random=~line+virus:line, data=sigma_small, family="multinomial2", prior=prior.sigma)
...
```{r sigma-3}
m.sigma.3<-MCMCglmm(cbind(infected, not_infected)~virus, random=~idh(virus):line, data=sigma_small, family="multinomial2", prior=prior.sigma)
...
```{r sigma-4}
m.sigma.4<-MCMCglmm(cbind(infected, not_infected)~virus, random=~idh(virus):line, rcov=~idh(virus):units, data=sigma_small, family="multinomial2", prior=prior.sigma)
...
```{r sigma-5}
m.sigma.5<-MCMCglmm(cbind(infected, not_infected)~virus, random=~us(virus):line, rcov=~idh(virus):units, data=sigma_small, family="multinomial2", prior=prior.sigma)
...
### Marginal Priors for Covariances and Correlations

Above, we derived prior specifications that resulted in marginal priors for the variances which are well understood. What do the marginal priors for the covariances and correlations look like under these prior specifications? For the inverse-Wishart, if $\texttt{V}$ is diagonal the marginal distribution of any correlation is beta distributed with shape and scale equal to $(\texttt{nu}-k+1)/2$ [@Barnard.2000]. The beta is flat when the shape=scale=1 implying that $\texttt{nu}=k+1$. As $\texttt{nu}$ becomes smaller the distribution becomes more and more U-shaped with a lot of prior density close to -1 and 1. As $\texttt{nu}$ increases beyond $k+1$ this distribution becomes more constrained around zero \@ref(fig:prior-cor). These results also hold for parameter-expanded priors. Previously, we used priors that had a marginal $F_{1,1}$ prior which involves setting $\texttt{nu}=k$ which has a U-shaped prior density for the correlations. Setting $\texttt{nu}=k+1$ induces a $F_{1,2}$ prior on the variances. 

Let's rerun model `m.sigma.5` with an $F_{1,2}$ prior of the same scale:

```{r sigma-6}
prior.sigma.6=list(R=IW(1, 0.002), G=F(2, 1000))

m.sigma.6<-MCMCglmm(cbind(infected, not_infected)~virus, random=~us(virus):line, rcov=~idh(virus):units, data=sigma_small, family="multinomial2", prior=prior.sigma.6)
```

The variance in $\texttt{line}$ effects over viruses remain largely unchanged

```{r }
plot(mcmc.list(m.sigma.5$VCV[,c(1,5,9)], m.sigma.6$VCV[,c(1,5,9)]))
```

The correlations in $\texttt{line}$ effects between viruses are nudged a little closer to zero under the new prior but the effect is rather subtle (there's also some auto-correltaion in chains so they should be run for longer than the number of default iterations). 

```{r }
plot(mcmc.list(posterior.cor(m.sigma.5$VCV[,1:9])[,c(2:3,6)], posterior.cor(m.sigma.6$VCV[,1:9])[,c(2:3,6)]))
```

Increasing $\texttt{nu}$ substantially to 11 has a small effect on the variances, with the prior favouring smaller values:

```{r sigma-7}
prior.sigma.7=list(R=IW(1, 0.002), G=F(10, 1000))

m.sigma.7<-MCMCglmm(cbind(infected, not_infected)~virus, random=~us(virus):line, rcov=~idh(virus):units, data=sigma_small, family="multinomial2", prior=prior.sigma.7)

plot(mcmc.list(m.sigma.5$VCV[,c(1,5,9)], m.sigma.7$VCV[,c(1,5,9)]))
```

but the correlations are now more obviously pushed towards zero.   

```{r }
plot(mcmc.list(posterior.cor(m.sigma.5$VCV[,1:9])[,c(2:3,6)], posterior.cor(m.sigma.7$VCV[,1:9])[,c(2:3,6)]))
```

While flat marginal priors for the correlations may seem like a desirable property...
