...
Above, we derived prior specifications that resulted in marginal priors for the variances which are well understood. What do the marginal priors for the covariances and correlations look like under these prior specifications?  For the inverse-Wishart prior, results are widely known. When matrix-$F$ priors were first available in $\texttt{MCMCglmm}$, very little was known about them, and my recommendations were mainly based on experience with analysing real and simulated data. Thankfully, analytical results are now available [@Mulder.2018][^mulder]. 
...
[^mulder]: To use results from that paper note that $\delta$, $\nu=\texttt{nu}$ and ${\bf B}=\texttt{V}$ and ${\bf \Psi}$ = $\texttt{alpha.V}/(\texttt{nu}+k-1)$
