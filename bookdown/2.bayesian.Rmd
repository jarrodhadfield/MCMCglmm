...
plot(dprior(xv, prior=list(V=1, nu=0.002))~xv, xlab=expression(sigma^2), ylab="Density", cex.lab=1.25, type="l")
```
...
prior.p<-function(par, priorB, priorR, log=FALSE){
    if(log){
      d<-dnorm(par[1], mean=priorB$mu, sd=sqrt(priorB$V), log=TRUE)+dprior(par[2], priorR, log=TRUE)
    }else{
      d<-dnorm(par[1], mean=priorB$mu, sd=sqrt(priorB$V))*dprior(par[2], priorR)
    } 
    return(d) 
}
```
...
V<-1
for(i in 1:4){
dv<-dprior(xv, prior=list(V=1, nu=nu[i]))
