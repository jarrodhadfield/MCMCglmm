...
``` {r ChickWeight, echo=FALSE, include=TRUE, fig.cap="Weight data of 50 chicks from hatching until three weeks old.", fig.width=9, fig.height=6}
...
This has the same form as Equation \@ref(eq:Vrr) although note that that it misses a term $\sigma_{\texttt{Time:units}, \texttt{units}}$ and so the residual variance, $\sigma^2_e$,is is forced to increase as $\texttt{Time}$ gets larger.[^covu2] 
...
The reason that the (co)variances of the $\texttt{Chick}$ effects has decreased is because the variability in weights is increasing over time, but our new model attributes part of this increase to things other than variation in growth rate. The model does a good job at predicting how the variance increases with $\texttt{Time}$, and in particular predicts the variance at hatching much better than our previous model (Figure \@ref(fig:pred-int2)).
...
If we overlay these predictions from our model that assumed a constant residual variance, we can see that the predictions for the mean are rather similar (Figure \@ref(fig:CWpred-3)). 
...
```
Widget 6.1: In red (wire-frame) are the correlations in the marginal residuals of weight over $\texttt{Time}$. In blue (solid) is the predicted correlations from a model with random chick intercept and slopes and a residual variance that is assumed to change with the square of $\texttt{Time}$. 
<details>
...
```{r ar-2d, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="", fig.width=9, fig.height=5}
par(mfrow=c(1,3))
plot(empiricalC[1,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 0", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.3[1,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC[6,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 10", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.3[6,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC[12,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 21", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.3[12,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
```
...
```{r CWresid-log, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="Weights of each chick as a function of age (points). Predictions are shown for models where the mean growth curve was fitted as a quadratic function of $$\\texttt{Time}$ with  random chick intercept and slopes. The residual variance is either assumed constant (red lines - model `mweight.2`) or to change with square of $\\texttt{Time}$ (blue lines - model `mweight.3`).", fig.width=9, fig.height=6}
xyplot(rep(0,nrow(ChickWeight))+I(log(weight)-pweight.5)+I(log(weight)-pweight.6)~Time|Chick, data=ChickWeight, distribute.type = TRUE, type   = c("l", "l", "l"), lwd=c(1, 2,2), col=c("black", "red", "blue"), ylab="Residual log(Weight)",  layout = c(10, 5), as.table = TRUE)
```
...
```{r label=ar-rgl-l5, echo=FALSE, include=TRUE, webgl=TRUE, cache=FALSE}
ChickWeight$marginal.resid.l<-c(log(ChickWeight$weight)-predict(mweight.5))
...
theoreticalC.5<-cov2cor(buildV(mweight.5, diag=FALSE)[1:12,1:12])
...
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =theoreticalC.5, add=TRUE,  col = "blue", alpha=0.9)
```
Widget log random regression

```{r ar-2d-l5, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="", fig.width=9, fig.height=5}
par(mfrow=c(1,3))
plot(empiricalC.l[1,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.5[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 0", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.5[1,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC.l[6,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.5[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 10", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.5[6,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC.l[12,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.5[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 21", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.5[12,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
```
...
``` {r ante, echo=TRUE}
prior.7<-list(R=list(V=diag(1), nu=0.002, beta.mu=rep(0,3), beta.V=diag(3)*10000))
mweight.7<-MCMCglmm(weight~Time+I(Time^2)+Diet:Time, rcov=~antec3v(as.factor(Time)):Chick, data=ChickWeight, prior=prior.7, pr=TRUE)

pweight.7<-predict(mweight.7, marginal=NULL)


prior.8<-list(R=IW(1, 0.002), G=list(G1=list(V=diag(1), nu=0.002, beta.mu=0, beta.V=10000), G2=IW(1, 0.002)))

mweight.8<-MCMCglmm(weight~Time+I(Time^2)+Diet:Time, random=~antec1v(as.factor(Time)):Chick+us(Time):units, data=ChickWeight, prior=prior.8, pr=TRUE)

#theoreticalC.7<-matrix(colMeans(mweight.7$VCV[,1:(12^2)]), 12,12)+diag(12)*mean(mweight.7$VCV[,1+(12^2)])
theoreticalC.7<-matrix(colMeans(mweight.7$VCV), 12,12)
```

```{r label=ar-rgl2, echo=FALSE, include=TRUE, webgl=TRUE, cache=FALSE}
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(empiricalC), xlab = "Time 1", ylab = "Time 2", zlab = "correlation", aspect = c(1, 1, 0.5), col = "black", front="lines", back="lines", diagonal = "back")
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(theoreticalC.7), add=TRUE,  col = "blue", alpha=0.9)
```
...
plot(ChickWeight$weight~ChickWeight$Time, ylim=range(ChickWeight$weight, lower.7, upper.7, lower.8, upper.8), ylab="Weight (g)", xlab = "Time (days)")
...
``` {r echo=TRUE, cache=TRUE}
prior.9<-list(R=list(V=diag(1), nu=0.002, beta.mu=rep(0,3), beta.V=diag(3)*10000))
mweight.9<-MCMCglmm(log(weight)~Time+I(Time^2)+Diet:Time, rcov=~antec3v(as.factor(Time)):Chick, data=ChickWeight, prior=prior.7, pr=TRUE)
...
```{r label=ar-rgl-l9, echo=FALSE, include=TRUE, webgl=TRUE, cache=FALSE}
ChickWeight$marginal.resid.l<-c(log(ChickWeight$weight)-predict(mweight.9))
chick.wide <- reshape(ChickWeight[,c("marginal.resid.l", "Chick", "Time")], v.names="marginal.resid.l", idvar="Chick", timevar="Time", direction="wide")
head(chick.wide[,1:5])
empiricalC.l<-cor(chick.wide[,-1], use="pairwise.complete.obs")
theoreticalC.9<-cov2cor(matrix(colMeans(mweight.9$VCV), 12,12))

persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =empiricalC.l, xlab = "Time 1", ylab = "Time 2", zlab = "correlation", aspect = c(1, 1, 0.5), col = "black", front="lines", back="lines", diagonal = "back")
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =theoreticalC.9, add=TRUE,  col = "blue", alpha=0.9)
```
Widget log ante

```{r ar-2d-l9, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="", fig.width=9, fig.height=5}
par(mfrow=c(1,3))
plot(empiricalC.l[1,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.9[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 0", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.9[1,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC.l[6,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.9[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 10", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.9[6,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC.l[12,]~unique(ChickWeight$Time), col="black", ylim=range(c(empiricalC.l[1,], theoreticalC.9[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 21", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.9[12,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
```
