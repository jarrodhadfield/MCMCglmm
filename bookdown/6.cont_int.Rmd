...
Because we can conceptualise the random regression in terms of the covariance between observations, rather than intercepts, slopes, and so on, random regression models are sometimes known as covariance function models.

The function `buildV` returns the predicted variance of observations (and the covariances if `diag=FALSE`). As with the predict function, there is the option to condition on some of the random effects (i.e. treat them as constants) by excluding them from the argument passed to marginal (see Section \@ref(ranpred-sec)). However, the default is not to condition on them and so we can get the total predicted variance for each observation due to chick effects and the residuals. In our model, the residual variance, $\sigma^2_{\textrm{units}}$, is constant and so we can simply add it to Equation \@ref(Vrr-eq) to get the total variance (after accounting for the fixed effects).

```{r }
predv.weight.2<-buildV(mweight.2)
```

The predicted variance goes up dramatically with $\texttt{Time}$ (red points in Figure \@ref(fig:V-rr)). In contrast, the model with chick intercepts only assumes that the variance is constant with $\texttt{Time}$ (black points in Figure \@ref(fig:V-rr)).
...
At this point we may be fairly chuffed with the random-intercept model - not only does tit seem to predict the expected response well, but in terms of the variance in the response, it's a massive improvement. However, it does raise an issue. If the model correctly specifies how chicks grow as a function of $\texttt{Time}$ this change in the variance is expected. However, it does mean that we could find evidence for variation in chick effects that comes solely from how the variance in weight changes as a function of $\texttt{Time}$ (i.e from Equation \@ref(eq:Vrr)) not from assessing whether weights from the same chick are correlated (i.e from Equation \@ref(eq:Crr)).  
