...
The reason that the (co)variances of the $\texttt{Chick}$ effects has dramatically decreased is because the variability in weights is increasing over time, but only part of this is a consequence of variation in growth rate across chicks. The model does a good job at predicting how the variance increases with $\texttt{Time}$, and in particular predicts the variance at hatching well  (Figure \@ref(fig:pred-int2)).
...
```{r }
ChickWeight$marginal.resid<-ChickWeight$weight-predict(mweight.3)
```
...
Note that we needed to specify `use="pairwise.complete.obs"` because some chicks have missing values at later times, presumably because they died - an issue I will return to in Section \@ref(missing-sec). The theoretical and observed correlations are shown in blue and red respectively (Widget 6.1)


```{r label=ar-rgl, echo=FALSE, include=TRUE, hide=TRUE, webgl=TRUE, cache=FALSE}
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =empiricalC, xlab = "Time 1", ylab = "Time 2", zlab = "correlation", aspect = c(1, 1, 0.5), col = "red", front="lines", back="lines", diagonal = "back")
...
Three dimensional plots can be confusing. We can also plot the correlations between a specific time-point and all others. These plots are sometimes referred to as variograms. In Figure \@ref(fig:ar-2d) we have the empirical (red circles) and theoretical (blue points) correlations for hatching ($\texttt{Time}=0$) the mid-point ($\texttt{Time}=10$) and the end ($\texttt{Time}=21$). Note that at all time points there is a discontinuity for the theoretical correlations when $\texttt{Time 1}=\texttt{Time 2}$ due to the residual variance. In the geostatistical literature this is often referred to as the 'nugget effect' and is visible in Widget 6.1 by viewing from the origin.

```{r ar-2d, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="", fig.width=9, fig.height=5}
par(mfrow=c(1,3))
plot(empiricalC[1,]~unique(ChickWeight$Time), col="red", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 0", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.3[1,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC[6,]~unique(ChickWeight$Time), col="red", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 10", xlab="Time 2", cex.lab=1.5)
points(theoreticalC.3[6,]~unique(ChickWeight$Time), col="blue", pch=16, cex=1.4)
plot(empiricalC[12,]~unique(ChickWeight$Time), col="red", ylim=range(c(empiricalC[1,], theoreticalC.3[1,])), ylab="Correlation in Marginal Residuals: Time 1 = 21", xlab="Time 2", cex.lab=1.5)
...
``` {r ante, echo=TRUE}
prior.7<-list(R=IW(1, 0.002), G=list(G1=list(V=diag(1), nu=0.002, beta.mu=0, beta.V=10000)))
mweight.7<-MCMCglmm(weight~Time+I(Time^2)+Diet:Time, random=~antec1v(as.factor(Time)):Chick, data=ChickWeight, prior=prior.7, pr=TRUE)
...
persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(empiricalC), xlab = "Time 1", ylab = "Time 2", zlab = "correlation", aspect = c(1, 1, 0.5), col = "red", front="lines", back="lines", diagonal = "back")
...
The variance in chick weight increased dramatically with $\texttt{Time}$ and it required effort to understand how this aspect of the data is captured by random-regression models.  We also saw that while modelling the change in variance accurately improved our models ability to capture general features of the data, it did compromised our ability to predict some aspects, such as the predicted means. In addition to the variance, the mean chick weight also increased dramatically in time, and we managed to model this fairly intuitively by fitting $\texttt{Time}$ and $\texttt{Time}^2$ in the fixed effect formula.  Let's plot the means and variances for each $\texttt{Diet}$/$\texttt{Time}$ combination.

```{r }
means<-with(ChickWeight, tapply(weight, Time, mean))
vars<-with(ChickWeight, tapply(weight, Time, var))
plot(vars~means)
```


```{r }
meansl<-with(ChickWeight, tapply(log(weight), Time, mean))
varsl<-with(ChickWeight, tapply(log(weight), Time, var))
plot(varsl~meansl)
```
