...
In Figure \@ref(fig:CWresid) we plotted a time series for the residuals of each chick. Neighbouring residuals were much more likely to have the same sign than a random pair of residuals, indicating autocorrelation in the residuals: two observations close in time are more similar to each other than our model predicts.    

```{r }

ChickWeight$resid<-ChickWeight$weight-predict(mweight.3)

chick.wide <- reshape(ChickWeight[,c("resid", "Chick", "Time")], v.names="resid", idvar="Chick", timevar="Time", direction="wide")


empiricalC<-cov(chick.wide[,-1], use="pairwise.complete.obs")


Z<-cbind(1, unique(ChickWeight$Time))

Vchick<-matrix(colMeans(mweight.3$VCV[,1:4]),2,2)
Vres<-diag(colMeans(mweight.3$VCV[,5:6]))


theoreticalC.3<-Z%*%Vchick%*%t(Z)+diag(mean(mweight.3$VCV[,5])+Z[,2]*mean(mweight.3$VCV[,6]))
...
# persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(theoreticalC.3), add=TRUE, col = "blue")
...
#persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(empiricalC), xlab = "Time 1", ylab = "Time 2", zlab = "correlation", aspect = c(1, 1, 0.5), col = "red", front="lines", back="lines", diagonal = "back")
#persp3d(x = unique(ChickWeight$Time), y = unique(ChickWeight$Time), z =cov2cor(theoreticalC.7), add=TRUE, col = "blue")
...
```{r }
pweightF.2<-predict(mweight.2)
e.2<-ChickWeight$weight-pweightF.2
predict(mweight.2)
```
