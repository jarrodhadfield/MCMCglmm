...
[^anteV]: To calculate the (co)variance matrix of the residuals (${\bf V}_e$) we can form two matrices: ${\bf V}_{\epsilon}$ is a diagonal matrix with the innovation variances along the diagonal. ${\bf B}$ is a matrix with $b_{ij}=\beta_{i|j}$. ${\bf V}_e$= $({\bf I}-{\bf B})^{-1}{\bf V}_\epsilon ({\bf I}-{\bf B})^{-\top}$
...
mlweight.4<-MCMCglmm(log(weight)~Time+I(Time^2)+Diet:Time, random=~idh(Diet):units+us(1+Time):Chick, rcov=~antec1v(as.factor(Time)):Chick, data=ChickWeight, prior=prior.lweight.4)
