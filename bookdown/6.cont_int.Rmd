...
Essentially, we are building a series of nested multiple regressions, where each variable is regressed on all previous values. The residual variances of these regressions - the variances of the  $\epsilon$'s  - are referred to as the innovation variances as they add new variation into the time series. For $T$ variables there are $T(T-1)/2$ regression parameters and $T$ innovations variances. If we put no constraint on these parameters and estimated all $T(T+1)/2$ of them, we would be estimating a fully unstructured covariance matrix as specified by `us`, albeit using a different parameterisation. 
 ...
 that have a natural starting point. 


is to imagine the variable at time $t$ being dependent on the same variable at some previous time, $t-1$, $t-2$ and so on. 
