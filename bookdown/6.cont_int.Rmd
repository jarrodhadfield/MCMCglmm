...
``` {r ChickWeight, echo=FALSE, include=TRUE, fig.cap="Weight data of 50 chicks from hatching until three weeks old.", fig.width=7, fig.height=8}
xyplot(weight~Time|Chick, data=ChickWeight, col="black")
```
...
``` {r CWpred-1, echo=FALSE, fig=TRUE, include=TRUE, fig.cap="Weights of each chick as a function of age (points). The mean growth curve was fitted as a quadratic function of $\\texttt{Time}$ with random chick intercepts (model `mweight.1`). The predicted weights are shown as lines.", fig.width=7, fig.height=8}
xyplot(weight+pweight.1~Time|Chick, data=ChickWeight, distribute.type = TRUE, type   = c("p", "l"), lwd=c(1, 2), col=c("black", "black"))
```
...
xyplot(rep(0,nrow(ChickWeight))+I(weight-pweight.2)+I(weight-pweight.3)~Time|Chick, data=ChickWeight, distribute.type = TRUE, type   = c("l", "l", "l"), lwd=c(1, 2,2), col=c("black", "red", "blue"), ylab="Residual Weight")
```


We are in a tricky situation. The model with non-constant residual variance does a better job at catching the patterns in the data. If it did not, the variance associated with the term `us(Time):units` should be zero, and it is clearly not. However, when we focus on specific aspects of the data (e.g. the predicted means) it does a poorer job.  The central issue here is that both models have their inadequacies, and without fixing them it might be hard to draw very firm conclusions.  While dissatisfying, it is the reality. The $\texttt{ChickWeight}$ data set has become something of a classic for illustrating the power of random regression models, yet the difficulties and subtleties of the approach are really covered.  
...
```{r }
means<-with(ChickWeight, tapply(weight, list(Diet, Time), mean))
vars<-with(ChickWeight, tapply(weight, list(Diet, Time), var))
plot(vars[1,]~means[1,], col=1)
for(i in 2:4){
points(vars[i,]~means[i,], col=i)
}
```


```{r }
lmeans<-with(ChickWeight, tapply(log(weight), list(Diet, Time), mean))
lvars<-with(ChickWeight, tapply(log(weight), list(Diet, Time), var))
plot(lvars[1,]~lmeans[1,], col=1)
for(i in 2:4){
points(lvars[i,]~lmeans[i,], col=i)
}
```
...
xyplot(rep(0,nrow(ChickWeight))+I(log(weight)-pweight.5)+I(log(weight)-pweight.6)~Time|Chick, data=ChickWeight, distribute.type = TRUE, type   = c("l", "l", "l"), lwd=c(1, 2,2), col=c("black", "red", "blue"), ylab="Residual log(Weight)")
```
...
sim.7<-simulate(mweight.7, nsim=1000, marginal=NULL)

interval.7<-sapply(unique(ChickWeight$Time), function(x){quantile(sim.7[which(ChickWeight$Time==x),], prob=c(0.025, 0.5, 0.975))})

lines(interval.7[2,]~unique(ChickWeight$Time), col="green", lty=3, lwd=2)
lines(interval.7[1,]~unique(ChickWeight$Time), col="green", lty=3, lwd=2)
lines(interval.7[3,]~unique(ChickWeight$Time), col="green", lty=3, lwd=2)
```
