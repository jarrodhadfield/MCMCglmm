...
Naively, we might conclude that phylogenetic effects are less important for Tyrant flycatchers than they are for birds as a whole. However, if we express the variances in the same units (mm$^2$ per million years), the difference is much more subtle with the phylogenetic variance being only about `r round(mean(100*(m.phylo$VCV[,"species"]/all.depth)/mean(m.tyrant$VCV[,"species"]/tyrant.depth)-100))% smaller in the Tyrant flycatchers.
