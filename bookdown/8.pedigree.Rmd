...
Phylogenies can also be expressed in tabular form, although only two columns are required because for species we only need to record a single 'parent' - an immediate ancestor of that species. In general, however, phylogenies are not expressed in this form presumably because it is hard to traverse phylogenies (and pedigrees) backwards in time when they are stored this way. For phylogenetic mixed models we generally only need to traverse phylogenies forward in time (if at all) but I have stuck with convention and used the `phylo` class from the `ape` package to store phylogenies. To illustrate, we can use the phylogeny of the world's birds available through the package $\texttt{clootl}$

```{r }
bird.tree<-clootl::extractTree()
```
...
The phylogeny is plotted in Figure \@ref(fig:phylo) although I have omitted the species names due to legibility.

``` {r phylo, echo=FALSE, include=TRUE, fig.cap="The phylogeny of the world's birds.", fig.width=9, fig.height=7}
plot(bird.tree, show.tip.label=FALSE)
```

We can also use the AVONET data-base, available through the package $\texttt{PNC}$, which contains information about the world's birds, including their tarsus lengths:

```{r }
bird.data<-PNC::AVONET
```

As with pedigrees, all species appearing in the data frame passed to `data` need to appear in the phylogeny. Typically, this will be the (extant) species at the tips of the phylogeny and so those species appearing in the data frame should appear in the `tip.label` element of the phylo object. The internal nodes in the phylogeny typically ancestral species. Data may also exist for these ancestral species, or even for species present at the tips but measured many generations before. It is possible to include these data as long as the phylogeny has labelled internal nodes. If nodes are unlabelled then $\texttt{MCMCglmm}$ names them internally using the default arguments of `makeNodeLabel` from `ape`. In pedigrees a single generation elapses between a parent and an offspring. However, in phylogenies a variable number of generations (or amount of time) may elapse between parents and their daughter species and this is represented by the length of the branch that connects to nodes. 


We can, if we wish, extract the phylogeny in tabular form. Let's do this for a subset of the taxa: Siberian Thrush (*Geokichla sibirica*), Blue tit (*Cyanistes caeruleus*), Tufted Puffin (*Fratercula cirrhata*), Common Crane (*Grus grus*) and Bean Goose (*Anser fabalis*). 
...
You will notice that the branch length for *Anser fabalis* is one. This is because there are no internal nodes between the root and the *Anser fabalis*, and we used the default `scale=TRUE` in the call to $\texttt{inverseA}$. `scale=TRUE` rescales the branch lengths by a constant so that the total branch length from the root to each tip is one. This is only possible if the tree is ultrametric  - the total branch length from the root to each tip is constant before rescaling. In fact the original $\texttt{bird.tree}$ obtained using `extractTree` was not exactly ultrametric - there were small differences in total branch lengths due to rounding errors and the the function $\texttt{force.ultrametric}$ from $\texttt{phyltools}$ had to be used.

Let's build a model for the tarsus lengths of the world's birds. We'll work on the log-scale since the lengths are heavily right skewed and span several orders of magnitude. However, before we proceed we need to do a bit of housekeeping  - the names in the data and phylogeny are formatted slightly differently - we need to replace the space between the generic and specific name with an underscore:
...
We also need to remove the column $\texttt{family}$ as $\texttt{MCMCglmm} is then expected a long-format multi-response model with the $\texttt{family}$ column specifying the distribution (see Section \@ref(sec-longwide)):


```{r }
bird.data<-bird.data %>% select(-"family")
```

Finally, there are `r sum(!bird.data$species%in%bird.tree$tip.label)` species that appear in the data base that are not represented in the phylogeny. In many cases this is probably due to taxonomic revisions, and the species do appear under a different name. Here, we will simply omit these species from the analysis.  We will also prune the phylogeny so that species for which we don't have tarsus lengths are dropped.
...
We are finally ready for the analysis:
...
The intercept in a phylogentic model is the estimated value for the ancestor of all sampled taxa - the root of the phylogeny. On the arithemtic scale an intercept of `r round(mean(m.pylo$Sol),2)` translates into `r round(mean(exp(m.pylo$Sol)),2)`mm - roughly the length of a Buzzard's tarsus, and probably not too different from *Archaeopteryx*. The phylogenetic variance is substantially larger than the residual variance. In general, comparing these two variances directly is not meaningful since the phylogenetic variance is in units of trait squared per unit time whereas the residual variance is in units of traits squared. Nevertheless, it is is common to treat them as comparable  


Since the tree was scaled to have unit length the total variance across species is expected to be the sum of the phylogenetic and residual variance, and the phylogenetic 
