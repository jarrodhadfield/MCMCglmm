...
different degrees. Some alternatives are listed in Table \@ref(tab:vfunction).
...
::: 
  ...
  : (#tab:vfunction) Different random effect specifications in `lmer`,
...
[^4.1]: Remember that a global intercept is not fitted by default for
...
[^4.2]: IMPORTANT: In versions $<$ 2.05 priors on each variance of an
...
[^4.3]: In versions $<2.18$ `cor` fitted what is now a `corg` structure.
