...
In the previous sections of this Chapter we've often thought about multi-response models in terms of regression. In some cases, the regression coefficient was different at different levels. For example, in Section \@ref(bern-gaus-sec) the regression within patients differed from the regression across patients. In other cases, the regression coefficient was only fitted for specific levels.  For example, in Section \@ref(covu-sec) the regression was only fitted across patients not within. Sometimes, however, we would like a model where the regression is held constant across two or more levels but is allowed to be different (or zero) at others.  
