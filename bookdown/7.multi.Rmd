...
We will consider two multi-response models. First, we will simultaneously model log($\texttt{bili}$) as Gaussian and $\texttt{ascites}$ as Bernoulli with threshold link (probit). As we will see, in multi-response models, modelling Bernoulli responses as $\texttt{family="threshold"}$ has some nice advantages. 
...
In the model with two Gaussian responses we also thought about the association between the two traits in terms of regression. If we follow the same recipe for the Bernoulii response ($\texttt{ascites}$) regressed on the Gaussian response (log($\texttt{bili}$)) we end up with the regression coefficients we would obtain from probit regression had we fitted log($\texttt{bili}$) as a predictor and also the true expected log($\texttt{bili}$) of each patient (which of course we don't know, but constitute the $\texttt{id}$ effects for that trait). We can see that the regression is stronger within individuals than between (Figure \@ref(pbc1-reg)). The change in risk of $\texttt{acites}$ when increasing a patient's log($\texttt{bili}$) by one unit is greater than the difference in risk than between two patient's that on average differ in log($\texttt{bili}$) by one unit. 
...
$E[x|l]$ is the mean of the conditional normal, $\mu_x+\frac{Cov(x,l)}{var(l)}(l-\mu_l)$. The expectation of this when $l>0$ is then $\mu_x+\frac{Cov(x,l)}{var(l)}(E[l|l>0]-\mu_l)$ and $E[l|l>0]$ can be obtained using the inverse Mills ratio. <details><summary>See code</summary>

```{r condE}
condE <- function(mu, V, c, greater=TRUE) {

  # bivariate normal with mean vector mu and covariance matrix V
  # expectation of 1st variable given 2nd variable is
  # > c (greater=TRUE) or < c (greater=FALSE)
...
```{r label=pbc1-reg2, echo=FALSE, include=TRUE, fig.cap="MCMC trace plot of the regression coefficient of log serum concentration of bilirunbin ($\\texttt{bili}$) on ascites presence ($\\texttt{ascites}$) as obtained from a multi-response model (red). ", fig.width=7, fig.height=5}
post.reg<-1:1000
...
for(i in 1:1000){
	...
	post.reg[i]<-condE(mu, V.id+V.units, c=0, greater=TRUE)-condE(mu, V.id+V.units, 0, greater=FALSE)
}

plot(as.mcmc(post.reg), col="red")
```



```{r biserial, echo=FALSE, include=TRUE, fig.cap="", fig.width=7, fig.height=5}
...
V.id<-matrix(colMeans(m.pbc1$VCV[,1:4]),2,2)                   
V.units<-matrix(colMeans(m.pbc1$VCV[,4+1:4]),2,2)                   

ell.id <- ellipse(V.id, centre = mu, level = 0.95, npoints = 361)
ell.units <- ellipse(V.units, centre = mu, level = 0.95, npoints = 361)

par(mfrow=c(1,2))

plot(ell.id, type = "n", xlab = "log(bili)", ylab = "ascites", xlim=1.1*range(c(ell.id[,"x"], ell.units[,"x"])), , ylim=1.1*range(c(ell.id[,"y"], ell.units[,"y"])))

polygon(ell.id, border = 1)                       
points(mu[1], mu[2], pch = 19)           
abline(h = 0, lty = 2) 
                  
polygon(ell.id[which(ell.id[, 2]>0),1],ell.id[which(ell.id[, 2]>0),2],             
        col    = rgb(0.2, 0.6, 0.8, 0.40),   
        border = NA)

abline(v=condE(mu, V.id, c=0, greater=TRUE))
abline(v=condE(mu, V.id, c=0, greater=FALSE))

plot(ell.units, type = "n", xlab = "log(bili)", ylab = "ascites", xlim=1.1*range(c(ell.id[,"x"], ell.units[,"x"])), , ylim=1.1*range(c(ell.id[,"y"], ell.units[,"y"])))

polygon(ell.units, border = 1)                       
points(mu[1], mu[2], pch = 19)           
abline(h = 0, lty = 2) 

polygon(ell.units[which(ell.units[, 2]>0),1],ell.units[which(ell.units[, 2]>0),2],             
        col    = rgb(0.2, 0.6, 0.8, 0.40),   
        border = NA)

abline(v=condE(mu, V.units, c=0, greater=TRUE))
abline(v=condE(mu, V.units, c=0, greater=FALSE))
```

## Covariances between random and residual terms ($\texttt{covu}$)

Occasionally, we need to link a set of random effects with a set of residuals. This most commonly occurs in multi-response models when we have repeated measures for one response but only single measures for another response. We can link the set of random effects appearing in the final random term of the `random` specification with the residuals appearing in the first residual term of the `rcov` specification.  The linking is specified by adding a `covu=TRUE` to the prior specification for the first residual term. For example, imagine the random effect model specification:
...
Occasionally, we need to force a common relationship between two sets of multiple random effects. To give a brief overview, imagine that we have individuals with repeat measures for variables $x$ and $y$ and these individuals can be grouped into families. We would like to know the regression of $y$ on $x$ but the issue is that the within-individual variance is believed to be simply measurement error. As in the $\texttt{covu}$ model what we would really like to do is regress an individuals true average value for $y$ on the individuals true average value for $x$. If we only had one set of random effects (e.g. $\texttt{individual}$) this would be straightforward because we could fit the $2\times 2$ covariance matrix for individual effects and obtain the regression as $\sigma_\texttt{individual:x, individual:y}/\sigma_\texttt{individual:x}$. With the two sets of random effects, $\texttt{individual}$ and $\texttt{family}$, we would need to somehow enforce  $\sigma_\texttt{individual:x, individual:y}/\sigma_\texttt{individual:x}$=$\sigma_\texttt{family:x, family:y}/\sigma_\texttt{family:x}$ unless we though the regression might differ at these two levels. The argument $\texttt{theta_scale}$ allows you to do this by specifying which fixed and/or random effects should be duplicated and scaled.


## Multinomial Models

Multinomial models are difficult - both to fit and interpret. This is particularly true when each unit of observation only has a single realisation from the multinomial. In these instances the data can be expressed as a single vector of factors, and the family argument can be specified as `categorical`. To illustrate, using a very simple example, we'll use data collected on 666 Soay sheep from the island of Hirta in the St. Kilda archipelago [@Clutton-Brock.2004 Table A2.5].
