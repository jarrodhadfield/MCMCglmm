...
## Wide versus Long Format {#sec-longwide}
...
```{r tufted-num, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=5, fig.cap="Number of prey items of different types in 150 randomly selected samples from Tufted Puffin chicks. The tick-marks on the x-axis delimit samples taken in the same year, and within years, samples are ordered by the proportion of prey items that are Pacific sand lance."}
...
sample_save<-sample(1:nrow(tufted_puffin), nsamples)
...
xmid <-barplot(t(tufted_puffin[sample_save,-c(1:2)]),  
        ...
        col         = rainbow(5),
        ylab        = "Counts",
        xlab        = "Sample", names.arg=rep("", nsamples), ylim=c(0,45))
...
However, there is tremendous variation in the total number of prey items per sample and it looks as if those samples with many prey items are dominated by Capelin. The multinomial ignores (conditions on) the total number of prey items and essentially works with the proportions of each prey item, as shown in Figure \@ref(fig:tufted-prop) for the 150 random samples. 


```{r tufted-prop, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=5, fig.cap="Proportion of prey items that are of each type in 150 randomly selected samples from Tufted Puffin chicks. The tick-marks on the x-axis delimit samples taken in the same year, and within years, samples are ordered by the proportion of prey items that are Pacific sand lance."}

xmid<-barplot(t(prop_counts),    
        beside      = FALSE,       
        col         = rainbow(5),
        ylab        = "Proportions",
        xlab        = "Sample", names.arg=rep("", nsamples), ylim=c(0,1.1))
...
With two $4\times 4$ covariance matrices, the model summary is pretty overwhelming. To get a feel for the broad patterns, I have summarised the two matrices in terms of correlations (upper-triangle), variances (diagonals) and covariances (lower-triangle) in Table \@ref(tab:puffin). The posterior means together with the 95% credible intervals (in square brackets) are reported.
...
for(i in 1:4){
	for(j in 1:4){
		if(j>i){
		   ...
		   V_year[i,j]<-sum.post(m.tufted$VCV[,(i-1)*4+j])
		   V_units[i,j]<-sum.post(m.tufted$VCV[,16+(i-1)*4+j])
		}
	}		
}
...
kable(V, "html", escape = FALSE, row.names = TRUE, caption = "Posterior mean and 95\\% credible intervals for the variances (diagonal), covariances (lower-triangle) and correlations (upper-triangle) in year effects (top) and sample effects (bottom) from model $\\texttt{m.tufted}$", label="puffin") %>%
...
There seems to be substantial variation, both between years and between samples, in the linear predictors for each prey item. Although credible intervals are wide, the generally pattern seems to be that the between year variation exceeds the between sample variation.  Correlations between the linear predictors for different prey items also have wide credible intervals, but in general they seem fairly small. The exception to this is between the $\texttt{Herring}$ and $\texttt{SandLance}$ predictors, where the correlation is moderately positive, particularly at the level of samples ($\texttt{units}$). 

Does the model fit the data? We can generate a draw from the posterior predictive distribution for the specific years sampled using the predict function and specifying $\texttt{marginal=NULL}$:

```{r }
sim_tufted<-simulate(m.tufted, marginal=NULL)
```

This is a vector of counts of length $n(K-1)$ where the first $n$ values are the counts (in this example) for Capelin and the second set of $n$ values are for Herring and so on. The counts for $\texttt{Other}$ are not returned but can be obtained by taking the difference between the total number of counts for each observation and the sum of the counts across the other $K-1$ categories.  If we plot the quantiles of the observed number of counts against the quantiles of the posterior prediction for the number of counts we see a very strong 1:1 relationship (Figure \@ref(fig:qq-counts)). 

```{r qq-counts, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=5, fig.cap="qq-plot of prey item counts from a posterior predictive simulation of model $\\texttt{m.tufted}$ (x-axis) versus the empirical quantiles (y-axis)."}

qqplot(unlist(tufted_puffin[,c("Capelin", "Herring", "Pollock", "SandLance")]), sim_tufted, xlab="Posterior Predictive Quantiles", ylab="Observed Quantiles")
abline(0,1)
```

However, this is rather misleading. The strong relationship may simply be driven by variation in the total number of counts per sample - something that is not modelled, but conditioned on. For this reason, I usually focus on the proportions when assessing the adequacy of the model. The qq-plot of the proportions shows a slightly noisier relationship, but still, it looks like the the marginal distribution of the data is captured well by the model (Figure \@ref(fig:qq-props)).  

```{r qq-props, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=5, fig.cap="qq-plot of prey item proportions from a posterior predictive simulation of model $\\texttt{m.tufted}$ (x-axis) versus the empirical quantiles (y-axis)."}

qqplot(unlist(tufted_puffin[,c("Capelin", "Herring", "Pollock", "SandLance")])/rowSums(tufted_puffin[,c("Capelin", "Herring", "Pollock", "SandLance", "Other")]), sim_tufted/rowSums(tufted_puffin[,c("Capelin", "Herring", "Pollock", "SandLance", "Other")]), xlab="Posterior Predictive Quantiles", ylab="Observed Quantiles")
abline(0,1)
```

While these qq-plots are good for assessing whether the model captures broad aspects of the data distribution, they don't really provide insight as to whether the finer patterns have been captured well - for example, if changes in the composition of the samples over time has been adequately modelled.  In Figure \@ref(fig:tufted-sim2) I have plotted the proportion of each prey item for 150 random chicks as obtained from the posterior simulation (top) and compared it to the actual diet compositions (bottom).  We can see, at least qualitatively, that the model seems to be picking up the structuring due to year rather well. Within years, we do not expect the posterior simulation to match the observation perfectly, since we are drawing a sample for a new chick within that year (the predict function always marginalises residual $\texttt{unit}$ effects). However, if the model is doing a good job the distribution of compositions within years should look reasonable. Although hard to judge, there appears to be no major discrepancies - the model does predict that samples tend to be dominated by a single prey item even if the dominating prey item varies to some degree across samples within a year.      


```{r tufted-sim2, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=7, fig.cap="A posterior predictive simulation (top) using model $\\texttt{m.tufted}$ for the proportion of prey items that are of each type in 150 randomly selected samples. On the bottom are the observed proportion of prey items. The tick-marks on the x-axis delimit samples taken in the same year, and within years, samples are ordered by the proportion of prey items that are Pacific sand lance. Note that $\\texttt{unit}$ effects are marginalised in the posterior predictive simulation and so we do not expect an exact correspondence between samples: only the *distributions* within and across years should match."}

sim_counts<-matrix(sim_tufted, nrow(tufted_puffin), 4)[sample_save,]
...
layout(matrix(1:3, ncol = 1), heights = c(1, 0.25, 1))  
par(oma = c(1, 0, 0, 0))                               
...
xmid <- barplot(t(sim_prop),
                ...
                axisnames = FALSE,
                ylab    = "Proportions")
legend("top", legend = colnames(prop_counts), fill = cols,
       ncol = 5,  cex=1.4, bg="white")


par(mar = c(0, 4, 0, 2))               
plot.new()
plot.window(xlim = range(xmid) + c(-0.5, 0.5), ylim = c(0, 1))

year_centres    <- tapply(xmid, years, mean)
year_separators <- tapply(xmid, years, max) + 0.5

axis(1, at = year_centres, labels = unique(years),
     tick = FALSE, line = -3)

axis(1, at = c(1, year_separators),
     labels = FALSE, tcl = -0.6, line = -3)
axis(1, at = c(1, year_separators),
     labels = FALSE, tcl = 0.6, line = -3)

par(mar = c(4, 4, 0, 2))               
barplot(-t(prop_counts[ , ncol(prop_counts):1 ]),
        space     = 0,
        col       = cols[ ncol(prop_counts):1 ],
        ylim      = c(-1.1, 0),
        axisnames = FALSE,
        ylab      = "Proportions")
```

While the pictures are pretty, we need to go beyond them to get a full understanding of what the model is telling us. With $K=5$ categories we have $K-1=4$ latent variables (traits) that are the log-odds ratio of observing a particular prey item versus the base-line prey item, in this case $\texttt{Other}$. Abbreviating the prey types by their initial, the latent variables for sample $i$ in year $j$ are

$$
\begin{array}{rcl}
l^{(\texttt{C-O})}_{ij} =& \textrm{log}\left(\frac{Pr(\texttt{C}_{ij})}{Pr(\texttt{O}_{ij})}\right)&= \beta^{(\texttt{C-O})}_0+u^{(\texttt{C-O})}_{j}+e^{(\texttt{C-O})}_{ij}\\
l^{(\texttt{H-O})}_{ij} =& \textrm{log}\left(\frac{Pr(\texttt{H-O}_{ij})}{Pr(\texttt{O}_{ij})}\right)&= \beta^{(\texttt{H-O})}_0+u^{(\texttt{H-O})}_{j}+e^{(\texttt{H-O})}_{ij}\\
l^{(\texttt{P-O})}_{ij} =& \textrm{log}\left(\frac{Pr(\texttt{P}_{ij})}{Pr(\texttt{O}_{ij})}\right)&= \beta^{(\texttt{P-O})}_0+u^{(\texttt{P-O})}_{j}+e^{(\texttt{P-O})}_{ij}\\
l^{(\texttt{S-O})}_{ij} =& \textrm{log}\left(\frac{Pr(\texttt{S}_{ij})}{Pr(\texttt{O}_{ij})}\right)&= \beta^{(\texttt{S-O})}_0+u^{(\texttt{S-O})}_{j}+e^{(\texttt{S-O})}_{ij}\\
\end{array}
$$

where the $\beta_0$ are trait-specific intercepts, the $u$ are trait-specific year effects and and the $e$ are trait-specific sample ($\texttt{unit}$) effects that capture overdispersion.  The effects for each trait are really comparisons against the base-line category, $\texttt{Other}$, and I have used a notation that makes this explicit, since it is easy to forget.  Nevertheless, it needs to be kept in mind. For example, let's imagine that Capelin and Herring appear in the diet completely independently of each other and so knowing there are many Capelin in a particular year is not informative about the abundance of Herring. It might then be tempting to assume that two sets of year effects, $u^{(\texttt{C-O})}$ and $u^{(\texttt{H-O})}$ are uncorrelated. However, they may be correlated through their shared dependence on $\texttt{Other}$ prey items. To understand this, I often find it easier to think about the multinomial as a multivariate Poisson which is then followed by normalisation. The ratio  of the probabilities, for example of Capelin versus $\texttt{Other}$, is also equal to the ratio of expected counts:

$$\frac{Pr(\texttt{C}_{ij})}{Pr(\texttt{O}_{ij})} = \frac{n_{ij}Pr(\texttt{C}_{ij})}{n_{ij}Pr(\texttt{O}_{ij})}=\frac{E[n^{(\texttt{C})}_{ij}]}{E[n^{(\texttt{0})}_{ij}]}$$


 where $n_{ij}$ is the total number of counts across all categories: $n_{ij}=n^{(\texttt{C})}_{ij}+n^{(\texttt{H})}_{ij}+n^{(\texttt{P})}_{ij}+n^{(\texttt{S})}_{ij}+n^{(\texttt{O})}_{ij}$. Sine a Poisson GLM with log-link involves linear models of the log expectation, we can interpret each parameter in the multinomial model as the difference in parameters from a bivariate Poisson model. For example, if we have log-linear models for the Capelin and $\texttt{Other}$ counts: 


$$
\textrm{log}\left(E[n^{(\texttt{C})}_{ij}]\right) = \beta^{(\texttt{C})}_0+u^{(\texttt{C})}_{j}+e^{(\texttt{C})}_{ij}\\
$$

$$
\textrm{log}\left(E[n^{(\texttt{O})}_{ij}]\right) = \beta^{(\texttt{O})}_0+u^{(\texttt{O})}_{j}+e^{(\texttt{O})}_{ij}\\
$$


then the parameters of the multinomial model are $\beta^{(\texttt{C-O})}_0 = \beta^{(\texttt{C})}_0-\beta^{(\texttt{O})}_0$, $u^{(\texttt{C-O})}_{j}=u^{(\texttt{C})}_{j}-u^{(\texttt{O})}_{j}$ and $e^{(\texttt{C-O})}_{ij}=e^{(\texttt{C})}_{ij}-e^{(\texttt{O})}_{ij}$. 

The assumption that the abundance of Capelin in a particular year is independent of the abundance of Herring implies that $COV(u^{(\texttt{C})}, u^{(\texttt{H})})=0$. However, the covariance of the year effects in the multinomial is  (for Capelin and Herring):

$$
\begin{array}{rl}
COV(u^{(\texttt{C-O})}, u^{(\texttt{H-O})})=&COV(u^{(\texttt{C})}-u^{(\texttt{O})}, u^{(\texttt{H})}-u^{(\texttt{O})})& \\
=&COV(u^{(\texttt{C})}, u^{(\texttt{H})})-COV(u^{(\texttt{C})}, u^{(\texttt{O})})-COV(u^{(\texttt{H})}, u^{(\texttt{O})}) +VAR(u^{(\texttt{O})})& \\
\end{array}
$$

and would remain non-zero even if $COV(u^{(\texttt{C})}, u^{(\texttt{H})})=0$. If the abundances of Capelin, Herring and $\texttt{Other}$ are *all* independent, the covariance simplifies to $VAR(u^{(\texttt{O})})$ reflecting the shared dependence of the two log-odds on the base-line category, $\texttt{Other}$. Similarly, the variance of year effects in the multinomial is (for Capelin):

$$
\begin{array}{rl}
VAR(u^{(\texttt{C-O})})=&VAR(u^{(\texttt{C})})+VAR(u^{(\texttt{O})})-2COV(u^{(\texttt{C})}, u^{(\texttt{O})})\\
\end{array}
$$

Even if the abundance of Capelin and $\texttt{Other}$ are independent across years, the variance in the log-odds depends on the year variance of both categories: $VAR(u^{(\texttt{C})})+VAR(u^{(\texttt{O})})$. 


When we fitted multi-response models previously we naturally assumed that a diagonal covariance matrix somehow represents a null - the two traits are independent of each other. However, the above results suggest that we may need to modify our null when thinking about multinomial models. If we think that the log abundances of different prey items are independent across years but have the same yearly variation, then our null for the covariance matrix of multinomial year effects would have $2\sigma^2_{u}$ along the diagonal and $\sigma^2_{u}$ on the off-diagonals. Looking at the estimates when we fitted a fully unstructured covariance matrix for the year effects (Table \@ref(tab:puffin)) suggest this model may not perform that well. While the between-year variances have large credible intervals, and so a common variance of around 20 may not be too bad, a common covariance of 10 (correlation of 0.5) looks too large. Nevertheless, we can fit this model and assess the fit. We can represent the covariance structure of the year effects in this model as ${\bf V}_{\texttt{year}} = \sigma^2_{u}({\bf I}+{\bf J})$ where ${\bf I}$ and ${\bf J}$ are $4\times 4$ identity and unit matrices, respectively[^unit]. To fit a year covariance matrix of this form we can use an $\texttt{idv}$ variance structure with model formula `trait+at.set(trait, 1:(K-1))`[^IJ]: 
...
[^IJ]: It is not immediately obvious what this is doing. The model formula specifies five parameters across the four traits.  The term $\texttt{trait}$ defines four effects, one effect for each trait since the intercept is suppressed for formula inside variance functions.  The term `at.set(trait, 1:4` sets up a single binary indicator variable with a one when the trait level is 1, 2, 3, or 4. Since this is all the four traits, this is just setting up a column of ones.  The associated parameter is common to all traits and this will be our $\texttt{Other}$ effect. If we interact the variance structure with year we end up with a set of five year effects for year $j$ with design matrix:
	...
	The $\texttt{idv}$ variance structure assumes the five effects are uncorrelated with a common variance, such that $\sigma^2_u{\bf Z}_j{\bf Z}^{\top}_j = \sigma^2_u({\bf I}+{\bf J})$: the specification is equivalent to just fitting four year effects with the desired covariance structure. 

```{r m-tuftedb}
m.tuftedb<-MCMCglmm(cbind(Capelin, Herring, Pollock, SandLance, Other)~trait-1, data=tufted_puffin, random=~idv(~trait+at.set(trait, 1:4)):year, rcov=~us(trait):units, family="multinomial", longer=20, prior=prior.tufted, pr=TRUE)
```

The posterior mean and 95% credible interval of $\sigma^2_u$ is `r sum.post(m.tuftedb$VCV[,1])`. In the fully unstructured model ($\texttt{m.tufted}$) the average of the four between-year variances halved was `r 
round(mean(m.tufted$VCV[,c(1,6,11,16)])/2,2)` and the average of the six between-year covariances was `r 
round(mean(m.tufted$VCV[,c(1:6)[-c(1,6,11,16)]]),2)` (see Table \@ref(tab:puffin)). The estimate of $\sigma^2_u$ can be seen as a compromise estimate of these ten parameters.  To assess whether this simpler model does a good job at capturing the differences between years we can decide on a suitable metric and compare the metric calculated on the observed data with that calculated on posterior predictive simulations. One possible metric would be to calculate the mean proportions of Capelin, Herring, Pollock and Sand lance across samples for each year and then take the variance in these year means for each prey item or the correlation between year means for pairs of prey items. Relating this metric to the parameters of the  model is not straight forward since the sample means for each year will, in part, reflect between sample variability, particularly in years where replication is low. In addition, the model works with log-probabilities, yet the described metric works with raw proportions since these are often zero and logging isn't an option. Despite the metric's limitations, I would still expect it to reveal major problems with the specified year effects.  


```{r tufted-sim3, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=7, fig.cap="The mean proportion of each prey item across samples in a given year were calculated for one thousand posterior predictive simulations from model $\\texttt{m.tuftedb}$. This histograms represent the posterior predictive distributions for the variances of the yearly means (diagonal) or the correlations in yearly means between prey items. The red lines are the values calculated from the data."}

nsim<-1000
nsample<-nrow(tufted_puffin)
size<-rowSums(tufted_puffin[,-c(1:2)])

sim_year_covs<-matrix(NA, nsim, 16)
obs_year_covs<-1:16

obs.y<-unlist(tufted_puffin[,c("Capelin", "Herring", "Pollock", "SandLance")])
obs.y<-obs.y/size

for(j in 1:4){
  for(k in 1:4){
    obs.mean.1<-tapply(obs.y[(j-1)*nsample+1:nsample], tufted_puffin$year, mean)
    obs.mean.2<-tapply(obs.y[(k-1)*nsample+1:nsample], tufted_puffin$year, mean)
    obs_year_covs[(j-1)*4+k]<-if(j!=k){cor(obs.mean.1, obs.mean.2)}else{var(obs.mean.1)}
  }
}

for(i in 1:nsim){

  sim.y<-simulate(m.tuftedb, marginal=NULL)

  sim.y<-sim.y/size

  for(j in 1:4){
	for(k in 1:4){
	  sim.mean.1<-tapply(sim.y[(j-1)*nsample+1:nsample], tufted_puffin$year, mean)
	  sim.mean.2<-tapply(sim.y[(k-1)*nsample+1:nsample], tufted_puffin$year, mean)
	  sim_year_covs[i,(j-1)*4+k]<-if(j!=k){cor(sim.mean.1, sim.mean.2)}else{var(sim.mean.1)}
	}
  }
}
...
The posterior means and 95% credible intervals for the year variances are displayed in table \@ref(tab:puffinc). Again, the credible intervals are wide and a common variance of around 20 may not be too bad for all categories except $\texttt{Other}$. The yearly variance for $\texttt{Other}$ is considerably smaller, which is to be expected since this represents the covariance between year effects in the fully unstructured model which were consistently less than half the variances. A reduced between-year variance for the $\texttt{Other}$ category seems biologically reasonable: since this category lumps 40 prey items which are likely to show some degree of independence in their abundances across years.  

```{r echo=FALSE, results="asis"}
Vc<-cbind(colMeans(m.tuftedc$VCV[,1:5]), HPDinterval(m.tuftedc$VCV[,1:5])[,1], HPDinterval(m.tuftedc$VCV[,1:5])[,2])
rownames(Vc)<-c("Capelin", "Herring", "Pollock", "SandLance", "Other")
colnames(Vc)<-c("Posterior Mean", "l-95% CI", "u-95% CI")

kable(Vc, "html", escape = FALSE, row.names = TRUE, caption = "Posterior mean and 95\\% credible intervals for the variances from model $\\texttt{m.tuftedc}$. Note that the variance labelled $\\texttt{Other}$ is the variance of the main year effects fitted as `~year`.", label="puffinc", digits = 2, format.args = list(nsmall = 2)) %>%
  kable_styling(full_width = TRUE, font_size=14)
```

If we perform the same posterior predictive check that we did for the previous model we can see it does a good job (Figure \@ref(fig:tufted-sim4)). 

```{r tufted-sim4, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=7, fig.cap="The mean proportion of each prey item across samples in a given year were calculated for one thousand posterior predictive simulations from model $\\texttt{m.tuftedc}$. This histograms represent the posterior predictive distributions for the variances of the yearly means (diagonal) or the correlations in yearly means between prey items. The red lines are the values calculated from the data."}

for(i in 1:nsim){

  sim.y<-simulate(m.tuftedc, marginal=NULL)
...
We could also consider a similar model for the sample $\texttt{unit}$ effects:

```{r m-tuftedd}
m.tuftedd<-MCMCglmm(cbind(Capelin, Herring, Pollock, SandLance, Other)~trait-1, data=tufted_puffin, random=~idh(trait):year+year+units, rcov=~idh(trait):units, family="multinomial", longer=20, prior=prior.tufted, pr=TRUE)
```

The magnitude of the units variances show similar patterns to the year variances, with $\texttt{Other}$ being small and Capelin being large (Table \@ref(tab:puffind)).

```{r echo=FALSE, results="asis"}
V_d<-matrix(NA, 5, 2)
for(i in 1:5){
	V_d[i,1]<-sum.post(m.tuftedd$VCV[,i])
	if(i!=5){
		V_d[i,2]<-sum.post(m.tuftedd$VCV[,i+6])
	}else{
		V_d[i,2]<-sum.post(m.tuftedd$VCV[,6])
	}	
}

rownames(V_d)<-c("Capelin", "Herring", "Pollock", "SandLance", "Other")
colnames(V_d)<-c("year", "units")

kable(V_d, "html", escape = FALSE, row.names = TRUE, caption = "Posterior mean and 95\\% credible intervals for the variances from model $\\texttt{m.tuftedd}$. Note that the variance labelled $\\texttt{Other}$ is the variance of the main year and unit effects fitted as `~year` and `~units` respectively.", label="puffind", digits = 2, format.args = list(nsmall = 2)) %>%
  kable_styling(full_width = TRUE, font_size=14)
```

Since model $\texttt{m.tuftedd}$ simplifies the covariance structure of the unit effects compared to previous models, it makes sense to assess the fit of the model by looking at unit-level patterns. To do this, we can simply calculate the proportion of each prey item in a sample and then calculate the variance in these proportions , or the correlation in proportions for pairs of prey items. Figure \@ref(fig:tufted-sim5) plots the posterior predictive distributions of these quantities and compares them to their observed values. 

```{r tufted-sim5, echo=FALSE, include=TRUE, fig.cap="", fig.width=9, fig.height=7, fig.cap="The histograms represent the posterior predictive distributions for the variances of the proportion of each prey item in a sample (diagonal) or the correlations in the proportion of prey items in a sample. The red lines are the values calculated from the data."}

sim_sample_covs<-matrix(NA, nsim, 16)
obs_sample_covs<-1:16

for(j in 1:4){
  for(k in 1:4){
    if(j!=k){
    	obs_sample_covs[(j-1)*4+k]<-cor(obs.y[(j-1)*nsample+1:nsample], obs.y[(k-1)*nsample+1:nsample])
	}else{
		obs_sample_covs[(j-1)*4+k]<-var(obs.y[(j-1)*nsample+1:nsample])}
   }
}

for(i in 1:nsim){

  sim.y<-simulate(m.tuftedd, marginal=NULL)
  ...
  for(j in 1:4){
	for(k in 1:4){
	  if(j!=k){
	    sim_sample_covs[i,(j-1)*4+k]<-cor(sim.y[(j-1)*nsample+1:nsample], sim.y[(k-1)*nsample+1:nsample])
	  }else{
		sim_sample_covs[i,(j-1)*4+k]<-var(sim.y[(j-1)*nsample+1:nsample])}
	  }
	}
}
...
for(j in 1:4){
  for(k in 1:4){
	  hist(sim_sample_covs[,(j-1)*4+k], breaks=30, main=if(j==k){c("Capelin", "Herring", "Pollock", "SandLance")[j]}else{""}, ylab="", xlab="")
	  abline(v=obs_sample_covs[(j-1)*4+k], col="red")
  }	
}
```

Generally, the model seems to predict these quantities well, although the correlation between Herring and Sand lance is at the extreme end of that seen in the posterior predictive simulations. This is evidenced in the fully unstructured model where the correlation is estimated to be more positive than all other correlations (Table \@ref(tab:puffin)).



## Zero-modified Poisson Models

A range of distributions are available for dealing with data where the non-zero counts conform to a Poisson distribution but the frequency of zeros is either too high or too low. The zero-truncated Poisson (`ztpoisson`) is the most extreme and is for situations where zeros are entirely missing. Three other distributions provide greater flexibility with the zero-inflated Poisson (`zipoisson`) allowing an excess of zeros and the hurdle Poisson (`hupoisson`) and the zero-altered Poisson (`zapoisson`) allowing both excesses and deficits. For these three distributions each data point is associated with two latent variables. The first latent variable is associated with a Poisson distribution and the second latent variable is associated with a Bernoulli distribution. For zero-inflated Poisson models the first latent variable specifies a standard Poisson, and the second latent variable models the probability that a zero belongs to this Poisson (failure) or is from some other process (success). The probability is modelled with logit link. For the hurdle and zero-altered Poissons, the first latent variable specifies a zero-truncated Poisson, and the second latent variable models the probability that an observation is a zero or not. For the hurdle Poisson a zero is a success and the probability is modelled with logit link. For the zero-altered Poisson a non-zero is a success and the complementary log-log link is used. Figure \@ref(fig:zm-poisson) provides a graphical representation of these distributions.


```{r zm-poisson, echo=FALSE, include=TRUE, fig.cap="", fig.width=7, fig.height=5, fig.cap="Probability mass function for a zero-modified Poisson. The blue and yellow bars combined represent a Poisson distribution with a mean of three. The red bar represents the excess zeros compared to what is expected under the Poisson (in yellow). The blue bars alone represent a zero-truncated distribution. In zero-inflated models, the first latent variable is the mean of the Poisson (blue and yellow bars combined) and the second latent variable models the probability of a zero being in the red category versus the yellow. In hurdle and zero-altered  models, the first latent variable is the mean of a zero-truncated Poisson (blue bars) and the second latent variable models the probability of a zero (red and yellow bars combined) versus non-zero (blue bars)."}

lambda<-3        
pinfl<-0.40 

x<-0:qpois(0.999, lambda)

p_pois <- dpois(x, lambda)

prob_mat <- rbind(c((1-pinfl)*p_pois[1], rep(0, max(x))), c(pinfl, rep(0, max(x))),c(0,(1-pinfl)*p_pois[-1]))

barplot(prob_mat,     
        col      = c("gold","firebrick", "steelblue"),
        names.arg = x,
        xlab     = "y",
        ylab     = "Probability",
)
...
To illustrate these models I'll use the data `bioChemists` available in the package $\texttt{pscl}$. The data were collected on 915 biochemistry graduates and the response variable `art` is the number of papers published during their PhD.   
...
Approximately, 30% of students do not publish a paper

```{r echo=TRUE}
table(bioChemists$art==0)/915
```

which is considerably more than expected from a Poisson distribution with a mean equal to the mean number of papers published (`r round(mean(bioChemists$art), 2)`).

```{r }
ppois(0,mean(bioChemists$art))
```

If we compare the observed frequencies of the number of papers published with a standard Poisson, we can see than not only is there an excess of zero's but also an excess of students publishing four or more papers (Figure \@ref(fig:art)).


```{r art, echo=FALSE, include=TRUE, fig.width=7, fig.height=5, fig.cap=paste0("Number of papers published by biochemistry graduate students during their PhD. The red dots are the probabilities calculated from a Poisson with a mean of ", round(mean(bioChemists$art), 2), ".")}

lambda<-mean(bioChemists$art)        

x<-0:max(bioChemists$art)

p_pois <- dpois(x, lambda)

mids<-barplot(table(factor(bioChemists$art, levels=x))/915, ylim=c(-0.01, max(p_pois)*1.1), xlab="Frequency", ylab="Papers published")

points(p_pois~mids[,1], pch=16, col="red")


dztpois<-function(x, lambda, log=FALSE){
	dpois(x, lambda, log=log)/(1-dpois(0, lambda))
}

ztloglik<-function(lambda, x){
-sum(dztpois(x, lambda, log=TRUE))
}

ztmean<-optim(1, x=bioChemists$art[which(bioChemists$art!=0)], ztloglik)$par

#points(I(dztpois(x[-1], ztmean)*(sum(bioChemists$art!=0)/915))~mids[-1,1], pch=16, col="blue")

```

While the excess of zeros may suggest that the distribution is zero-inflated, general overdispersion will also create an excess of zeros. If we focus only on the non-zero counts and compare their distribution to what we would expect under a zero-truncated Poisson, an excess of small values and very large values is evident - the non-zero data are overdispersed with respect to the zero-truncated Poisson (Figure \@ref(fig:art2)). As is often the case, it's unclear whether the large number of zero's in the data are due to zero-inflation or overdispersion more generally.

```{r art2, echo=FALSE, include=TRUE, fig.width=7, fig.height=5, fig.cap=paste0("Number of papers published by biochemistry graduate students during their PhD, excluding those students that published no papers. The blue dots are the probabilities calculated from a zero-truncated Poisson with a mean of ", round(mean(bioChemists$art[which(bioChemists$art!=0)]), 2), ".")}


x<-1:max(bioChemists$art)
zttab<-prop.table(table(factor(bioChemists$art[which(bioChemists$art!=0)], levels=x)))


mids<-barplot(zttab, xlab="Frequency", ylab="Papers published")


points(dztpois(x, ztmean)~mids[,1], pch=16, col="blue")

```

As with standard Poisson models, by allowing observation-level ($\texttt{unit}$) effects, MCMCglmm fits an overdispersed (zero-truncated) Poisson when fitting zero-modified Poisson distributions (Section \@ref(addod-sec)). Consequently, any zero-modification is with respect to the overdispersed distribution. As in Bernoulli models, $\texttt{unit}$ effects are fitted in the zero-modification model, but their variance is not identifiable and so should usually be fixed in the prior (Section \@ref(bernoulli-sec)). Similarly, any covariation between the Poisson model and the zero-modification model at the level of an observation is not identifiable, since non-zero Poisson counts are absent for all observations that have zero counts, obviously. Consequently, the residual covariance matrix for the pair of latent variables is usually constrained to have the form:
...
where $\sigma^2_{y}$ is the units variance for the (zero-truncated) Poisson. This can be achieved by using an $\texttt{idh}$ structure and fixing the second variance to one by specifying $\texttt{fix}$ in the prior.

### Zero-inflated Poisson

To the biochemsirty student data, we can fit a zero-inflated model where we specify constant odds for a zero belonging to the inflation process rather than the Poisson process, but the mean of the Poisson may depend on a suite of predictors such as the categorical predictors sex ($\texttt{fem}$) and marital status ($\texttt{Married}$) and continuous predictors such as the number of children aged 5 or younger ($\texttt{kid5}$) prestige of the department ($\texttt{phd}$) and how many papers the student's mentor published over the same period ($\texttt{ment}$).

```{r zip}
prior.zip=list(R=list(V=diag(2), nu=0.002, fix=2))
m.zip<-MCMCglmm(art~trait-1+at.level(trait,1):(fem + mar + kid5 + phd +ment), rcov=~idh(trait):units, data=bioChemists, prior=prior.zip, family="zipoisson", longer=10)
```
...
The first 'trait' has the same name as the response ($\texttt{art}$ in this example) and is the latent variable for the Poisson process. The second 'trait' is the latent variable for the zero-inflation and has the same name as the response prefixed by $\texttt{zi_}$ ($\texttt{zi_art}$). We can see that there is good evidence for overdispersion: the $\texttt{units}$ variance for the Poisson process ($\texttt{traitart.units}$) is far from zero and several of the covariates seem to impact the Poisson mean. However, note that the log odds of a zero belonging to the zero-inflation process is extremely small (`r round(mean(m.zip$Sol[,"traitzi_art"]), 2)`) - essentially a probability of zero - and the mixing is very poor for this parameter which has a very low effective sample size. We can see this visually by looking at the MCMC traces from the model (Figure \@ref(fig:mcmc-zip)). 

```{r label=mcmc-zip, echo=FALSE, include=TRUE, fig.cap="MCMC traces for the first four fixed effects from the zero-inflated Poisson model `m.zip` in which trait 1 ($\\texttt{art}$) is the Poisson process and trait 2 ($\\texttt{zi.art}$) is the zero-inflation.", fig.width=7, fig.height=8}
plot(m.zip$Sol[,1:4], density=FALSE)
```

The reason that the model mixes so poorly for the zero-inflation parameter is that there is essentially no zero-inflation.  We have complete separation where the likelihood is favouring estimates of the probability that are close to zero and therefore minus infinity on the logit scale (Section \@ref(sec-complete-separation)). Before worrying about zero-inflation I usually fit a standard (over-dispersed) Poisson: 


```{r }
prior.odp=list(R=list(V=diag(1), nu=0.002))
m.odp<-MCMCglmm(art~fem + mar + kid5 + phd + ment, data=bioChemists, prior=prior.odp, family="poisson")
```

and then use posterior predictive checks to see how many zero's you would expect under this simpler model:

```{r ppzi, fig.width=7, fig.height=5, fig.cap="Posterior predictive distribution from an (overdispersed) Poisson model fitted to the number of biochemistry students (out of 915) that publish no papers during their PhD. The red line is the observed number."}

# simulate from model
ysim<-simulate(m.odp, nsim=1000)

# count the number of zeros for each simulated data set
nzsim<-colSums(ysim==0)

# plot posterior predictive distribution with observed value
hist(nzsim, main="", xlab="Number of zeros")
abline(v=sum(bioChemists$art==0), col="red", lwd=2)
```

The simpler model seems to be entirely consistent with the number of zeros (Figure \@ref(fig:ppzi)) suggesting that a zero-inflated model may not be required.

### Hurdle Poisson models {#Hurdle}

In hurdle and zero-altered models, it is the probability of a zero versus non-zero that is modelled, rather than the probability a zero belongs to the zero-inflation process (Figure \@ref(fig:zm-poisson)). Consequently, complete separation only occurs when no zeros are observed and the probability tends to zero. The equivalent model to $\texttt{m.zip}$ but using a hurdle distribution therefore behaves better


```{r hup}
m.hup<-MCMCglmm(art~trait-1+at.level(trait,1):(fem + mar + kid5 + phd +ment), rcov=~idh(trait):units, data=bioChemists, prior=prior.zip, family="hupoisson", longer=10)
summary(m.hup)
```

Compared to the zero-inflated traces plotted in Figure \@ref(fig:mcmc-zip) the equivalent parameters in the hurdle model show better mixing (Figure \@ref(fig:mcmc-hup)).


```{r label=mcmc-hup, echo=FALSE, include=TRUE, fig.cap="MCMC traces for the first four fixed effects from the hurdle Poisson model `m.hup` in which trait 1 ($\\texttt{art}$) is the Poisson process and trait 2 ($\\texttt{zi.art}$) is the probability of being zero.", fig.width=7, fig.height=8}
plot(m.hup$Sol[,1:4], density=FALSE)
```

However, the interpretation of the hurdle model is slightly different than the zero-inflated model. In the hurdle model, fitting just an intercept for the zero-modification process implies that the proportion of zeros observed across different combinations of fixed effects is constant. Our 95% credible intervals for this proportion is (See section \@ref(binom-pred-sec)):

```{r echo=TRUE}
c2<-(16*sqrt(3)/(15*pi))^2
HPDinterval(plogis(m.hup$Sol[,"traithu_art"]/sqrt(1+c2))) 
```

While this is consistent with the overall frequency of zeros in the data set it is unclear whether it is consistent with the observed frequency of zeros across different groups of students. For example, the frequency of zero's is perhaps lower in men than women and increases with the number of children under 5 (there is only one observation for a women with 3 children under 5). 

```{r }
tapply(bioChemists$art==0, list(bioChemists$fem, bioChemists$kid5), mean)
```

In standard and zero-inflated Poisson models, these frequencies would be expected to change depending on how the predictors affect the mean number of papers (including zeros). For example, when we fitted the standard Poisson model to these data the predicted frequency of zeros is:

```{r echo=TRUE}
ysim<-simulate(m.odp, nsim=1000)==0
ysim<-apply(ysim, 1, sum)/1000

tapply(ysim, list(bioChemists$fem, bioChemists$kid5), mean)
```

which largely recapitulates what is seen in the data.  We can relax the assumption of a fixed proportion of zeros by fitting the same predictors for the zero-modified process 

```{r hup2}
m.hup2<-MCMCglmm(art~trait-1+trait:(fem+mar+kid5+phd+ment), rcov=~idh(trait):units, data=bioChemists, prior=prior.zip, family="hupoisson", longer=10)
summary(m.hup2)
```

However, the fully parameterised model involves almost doubling the number of location parameters. This highlights one of the disadvantages of hurdle models. If zeros do arise through the same process that generate the non-zero counts (either with zero-inflation or not) then the proportion of zero's observed across a set of explanatory variables will vary if those predictors affect the expectation of the Poisson. It may then be necessary to fit an equally complicated model for both processes even though no additional parameter would be required if the model was fitted as a standard Poisson and a single intercept parameter may suffice in a zero-inflated model if there were excess zeros. 


### Zero-altered Poisson Models {#ZAP}

Zero-altered Poisson (ZAP) models are identical to Poisson hurdle models except the second latent variable models the probability of a non-zero (rather than a zero) and the complementary log-log link is used[^zap]. The motivation behind this is that the standard Poisson can then become a special case of the zero-altered Poisson. With complementary log-log link, the probability of a zero is equal to $exp(-exp(l_2))$, where $l_2$ is the second latent variable. The probability of obtaining a zero from a Poisson distribution with mean $\lambda$ is $exp(-\lambda)$. Consequently, in a Poisson GLM with log-link, the probability of a zero is $exp(-exp(l_1))$, where $l_1$ is the first latent variable. Applying the constraint that $l_1=l_2$ across all observations, the zero-altered Poisson model is equivalent to the standard Poisson. In MCMCglmm, $l_1$ and $l_2$ cannot be constrained to be equal because they must at least differ in their residual, since each 'trait' must have a unique residual. Nevertheless, the residuals can be forced to have the same residual ($\texttt{unit}$) variance, although all of the information comes from the Poisson part. 

[^zap]: I can now find no reference of this model being referred to as a zero-altered model, but for back compatibility it will have to remain as `zapoisson`. 

```{r zap1, echo=TRUE, cache=TRUE}
m.zap1<-MCMCglmm(art~fem+mar+kid5+phd+ment, rcov=~trait:units, data=bioChemists, family="zapoisson", prior=prior.odp, longer=10)
summary(m.zap1)
```

If we plot the coefficients and $\texttt{units}$ variance for this model and compare it to the standard (overdispersed) Poisson we see that the posteriors are in close agreement, for all but the intercept (Figure \@ref(fig:mzap1)).  The difference in intercept presumably arises because of the separate $\txttt{unit}$ effect for the two processes. 

```{r label=mzap1, echo=FALSE, include=TRUE, fig.cap="In red is are the MCMC traces for the fixed effects and $\texttt{units}$ variance from the zero-inflated Poisson model `m.zap1` in which trait 1 ($\\texttt{art}$) is the Poisson process and trait 2 ($\\texttt{zi.art}$) is the probability of being zero. The MCMC traces in black are from the equivalent overdispersed Poisson model, `m.odp`.", fig.width=7, fig.height=8}

mcmc.1<-as.mcmc(cbind(m.odp$Sol[,1:6], m.odp$VCV[,1]))
mcmc.2<-as.mcmc(cbind(m.zap1$Sol[,1:6], m.zap1$VCV[,1]))
plot(mcmc.list(mcmc.1, mcmc.2), density=FALSE)
```

Consequently, we can test for zero-modification by setting up the contrasts so that the zero-altered regression coefficients are expressed as differences from the Poisson regression coefficients. When this difference is zero the variable causes no zero-modification, when it is negative it causes zero-inflation and when it is positive it causes zero-deflation:

```{r zap2}
m.zap2<-MCMCglmm(art~trait*(fem +mar + kid5 + phd + ment), rcov=~trait:units, data=bioChemists, prior=prior.odp, family="zapoisson", longer=10)
summary(m.zap2)
```

The credible intervals on the interaction coefficients (including `traitza_art`) are large and so drawing firm conclusions is hard, although there does seem that the more papers a mentor produces, the more zero-deflation (or conversely the less papers a mentor produces, the more zero-inflation).

## Zero-modified Binomial Models
