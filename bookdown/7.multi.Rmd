...
A simple regression suggests the two types of spending are negatively related but the association is weak with the $R^2$ close to zero.
...
We may be inclined to stop there, but lets proceed with a multi-response model of the problem. The two responses are passed as a matrix using `cbind()`, and the rows of this matrix are indexed by the reserved variable `units`, and the columns by the reserved variable `trait`.
...
``` {r multi.1, echo=TRUE}
prior.m5a.2<-list(R=list(V=diag(2), nu=1.002),
                  G=list(
                  	G1=list(V=diag(2), nu=2, alpha.mu=rep(0,2), alpha.V=diag(2)*1000)
                  ))

m5a.2<-MCMCglmm(cbind(y.hol, y.car)~trait-1, random=~us(trait):id, rcov=~us(trait):units, data=Spending, prior=prior.m5a.2, family=c("gaussian", "gaussian"))
```
...
The regression coefficients (see Figure \@ref(fig:asUV)) differ substantially at the within individual (green) and between
...
``` {r echo=TRUE, eval=TRUE}
Spending$y.hol2<-rnorm(200,0,sqrt(2))[Spending$id]+rnorm(800,0,1)
Spending$y.car2<-Spending$y.hol2*-0.3+rnorm(200,0,1)[Spending$id]+rnorm(800,0,sqrt(2))
```

We can fit the univariate and multivariate models to these data, and compare the regression coefficients as we did before. Figure \@ref(fig:MVvUV2) shows that the regression coefficients are all very similar and a value of -0.3 has a reasonably high posterior probability. However, it should be noted that the posterior standard deviation is smaller in the simpler model because the more strict assumptions have allowed us to pool information across the two levels to get a more precise answer.

``` {r echo=FALSE, eval=TRUE, cache=TRUE}
m5a.3<-MCMCglmm(y.car2~y.hol2, random=~id, data=Spending)
m5a.4<-MCMCglmm(cbind(y.hol2, y.car2)~trait-1, random=~us(trait):id, rcov=~us(trait):units, data=Spending, family=c("gaussian", "gaussian"))
```
...
[^6.1]: Earlier versions of the CourseNotes had `nu=1.002`. In versions $<$`<!-- -->`{=html}2.05 the marginal prior of a variance associated with an `idh` structure was inverse-Wishart with $\texttt{nu}^{\ast}=\texttt{nu}-1$ where $\texttt{nu}^{\ast}$ is the marginal degree of belief. In versions $>=$`<!-- -->`{=html}2.05 I changed this so that $\texttt{nu}^{\ast}=\texttt{nu}$ as it was leading to confusion.
