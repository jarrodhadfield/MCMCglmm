...
In the previous section we generated a long-format data-frame $\texttt{pbcseq_long}$ with two traits: $\texttt{status}$ first with three levels and then the continuous variable $\texttt{bili}$. In reality, $\texttt{status}$ is not a repeat-measure trait, it is the final outcome (censored, transplant or dead) duplicated across all of the patients records. Consequently we should only retain a single record. 
