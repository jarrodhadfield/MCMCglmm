...
prob_mat <- rbind(c((1-pi)*p_pois[1], rep(0, max(x))), c(pi, rep(0, max(x))),c(0,(1-pi)*p_pois[-1]))
...
```{r art, echo=FALSE, include=TRUE, fig.width=7, fig.height=5, fig.cap=paste("Number of papers published by biochemistry graduate students during their PhD. The red dots are the probabilities calculated from a Poisson with a mean of", round(mean(bioChemists$art), 2), ".")}
...
While the excess of zero's may suggest that the distribution is zero-inflated, general overdispersion will also create an excess of zeros compared to the standard Poisson. If we focus only on the non-zero counts and compare their distribution to what we would expect under a standard zero-truncated Poisson, an excess of small values and very large values is evident - the non-zero data are overdispersed with respect to the zero-truncated Poisson (Figure \@ref(fig:art2)). As is often the case, it's unclear whether the large number of zero's in the data are due to zero-inflation or overdispersion more generally.

```{r art2, echo=FALSE, include=TRUE, fig.width=7, fig.height=5, fig.cap=paste("Number of papers published by biochemistry graduate students during their PhD, excluding those students that published no papers. The blue dots are the probabilities calculated from a zero-truncated Poisson with a mean of", round(ztmean, 2), ".")}
...
where $\sigma^2_{y}$ is the units variance for the Poisson, or zero-truncated Poisson for the hurdle Poisson. This can be achieved by using an $\texttt{idh}$ structure and fixing the second variance to one by specifying $\texttt{fix}$ in the prior.

### Zero-inflated Poisson

We can fit a zero-inflated model to the biochemsirty student data where there is a constant odds of a zero belonging to the inflation process rather than the Poisson process, but the mean of the Poisson may depend on a suite of predictors such as the categorical predictors sex ($\texttt{fem}$) and marital status ($\texttt{Married}$) and continuous predictors such as the number of children aged 5 or younger ($\texttt{kid5}$) prestige of the department ($\texttt{phd}$) and how many papers the student's mentor published over the same period ($\texttt{ment}$).
...
The first 'trait' has the same name as the response ($\texttt{art}$ in this example) and is the latent variable for the Poisson process. The second 'trait' is the latent variable for the zero-inflation and has the same name as the response prefixed by $\texttt{zi_}$ ($\texttt{zi)art}$). Let's look at the model summary:

```{r }
summary(m.zip)
```

We can see that there is good evidence for overdispersion: the $\texttt{units}$ variance for the Poisson process ($\texttt{traitart.units}$) is far from zero and several of the covariates seem to impact the Poisson mean. However, note that the log odds of a zero belonging to the zero-inflation process is extremely small (`r round(mean(m.zip$Sol[,"traitzi_art"]), 2)`) - essentially a probability of zero - and the mixing is very poor for this parameter with a very low effective sample size. We can see this visually by looking at the MCMC traces from the model (Figure \@ref(fig:mcmc-zip)). 

```{r label=mcmc-zip, echo=FALSE, include=TRUE, fig.cap="MCMC traces for the first four fixed effects from model `m.zip` in which trait 1 ($\\texttt{art}$) is the Poisson process and trait 2 ($\\texttt{zi.art}$) is the zero-inflation.", fig.width=7, fig.height=8}
...
```{r fig.width=7, fig.height=5, fig.cap="Posterior predictive distribution from an (overdispersed) Poisson model fitted to the number of biochemistry students (out of 915) that publish no papers during their PhD. The red line is the observed number."}
...
The simpler model seems to be entirely consistent with the number of zeros suggesting that a zero-inflated model may not be required.
...
m.hup<-MCMCglmm(art~trait-1+at.level(trait,1):fem + at.level(trait,1):mar + at.level(trait,1):kid5 + at.level(trait,1):phd + at.level(trait,1):ment, rcov=~idh(trait):units, data=bioChemists, prior=prior.zip, family="hupoisson")
```
...
```{r echo=TRUE}
HPDinterval(ppois(0, exp(m.hup$Sol[,1]+m.hup$Sol[,3]+0.5*m.hup$VCV[,1])))  
```
