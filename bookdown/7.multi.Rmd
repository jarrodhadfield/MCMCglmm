...
```{r tufted-mm, echo=FALSE}

seabird<-read.csv("https://www.sciencebase.gov/catalog/file/get/64134381d34eb496d1ce3c82?f=__disk__00%2F91%2F53%2F009153966acb7a5d106004a9a01fcfdfb4504399")

tufted<-subset(seabird, BirdSpecies=="TUPU")

tufted$prey<-tufted$CommonName
tufted$prey[!tufted$prey%in%c("Walleye pollock","Pacific herring","Pacific sand lance","Capelin")]<-"Other"      

tufted_mm<-as.data.frame.matrix(table(tufted$SampleReference, tufted$prey))
tufted_mm$year<-tufted$CollectionYear[match(rownames(tufted_mm), tufted$SampleReference)]
tufted_mm$month<-tufted$CollectionMonth[match(rownames(tufted_mm), tufted$SampleReference)]

colnames(tufted_mm)<-gsub("Walleye pollock", "Pollock", colnames(tufted_mm))
colnames(tufted_mm)<-gsub("Pacific herring", "Herring", colnames(tufted_mm))
colnames(tufted_mm)<-gsub("Pacific sand lance", "SandLance", colnames(tufted_mm))
rownames(tufted_mm)<-NULL
```

To explore multinomial models we will analyse data collected by the Alaska Science Center on the prey items of seabirds breeding on Middleton Island in the Gulf of Alaska (soon to be renamed the Gulf of Canada) - [see here](https://www.sciencebase.gov/catalog/item/64134381d34eb496d1ce3c82). We will restrict ourselves to data obtained on the chicks of the wonderful Tufted Puffin. `r length(unique(seabird$CommonName))` different prey items were recorded but we will lump all but the top four prey items (Walleye pollock, Pacific herring, Pacific sand lance and Capelin) into a single category, $\texttt{other}$. After a little formatting we have the data-frame $\texttt{tufted_mm}$:

```{r }
head(tufted_mm)
```

Each row corresponds to one of `r nrow(tufted_mm)` samples and the first 5 columns are the counts of each prey item in the sample. The final two columns are the collection year (of which there are `r length(unique(tufted_mm$CollectionYear))`) and collection month (July or August). Figure \@ref(fig:tufted-num) plots the number of prey items for 150 random samples. 
...
sample_save<-sort(sample(1:nrow(tufted_mm), 150))
xmid <-barplot(t(tufted_mm[sample_save,c("Other", "Herring", "Pollock", "SandLance", "Capelin")]),  
        beside      = FALSE,        
        col         = rainbow(5),
        ylab        = "Counts",
        xlab        = "Sample", names.arg=rep("", 150), ylim=c(0,50))

years           <- tufted_mm$year[sample_save] 
...
legend("top",                       
       legend = c("Other", "Herring", "Pollock", "SandLance", "Capelin"),
       fill   = rainbow(5),
       ncol   = 5) 
```
...
```{r }
colMeans(tufted_mm[,c("Other", "Herring", "Pollock", "SandLance", "Capelin")])
```
...
tot_counts<-rowSums(tufted_mm[sample_save,c("Other", "Herring", "Pollock", "SandLance", "Capelin")])

xmid<-barplot(t(tufted_mm[sample_save,c("Other", "Herring", "Pollock", "SandLance", "Capelin")]/tot_counts),    
        beside      = FALSE,       
        col         = rainbow(5),
        ylab        = "Counts",
...
```{r }
tot_counts<-rowSums(tufted_mm[,c("Other", "Herring", "Pollock", "SandLance", "Capelin")])
colMeans(tufted_mm[,c("Other", "Herring", "Pollock", "SandLance", "Capelin")]/tot_counts)
```
...
tot_counts<-rowSums(tufted_mm[sample_save,c("Other", "Herring", "Pollock", "SandLance", "Capelin")])
prop_counts<-colMeans(tufted_mm[sample_save,c("Other", "Herring", "Pollock", "SandLance", "Capelin")]/tot_counts)
sim_counts<-t(sapply(tot_counts, FUN=function(x){rmultinom(1, prob=prop_counts, size=x)}))
...
m.tufted<-MCMCglmm(cbind(Herring, Pollock, Capelin, SandLance, Other)~trait-1, data=tufted_mm, random=~us(trait):year, rcov=~us(trait):units, family="multinomial", longer=20, prior=prior.tufted, pr=TRUE)
```
...
The difficulty, I think, is that the effects are comparisons with the base-line category, yet it is easy to forget this. For example, let's imagine that Pollock and Herring appear in the diet completely independently of each other and so knowing there are many Pollock in a particular year is not informative about the abundance of Herring. It might then be tempting to assume that two sets of year effects, $u_{\texttt{P}}$ and $u_{\texttt{H}}$ are uncorrelated. However, they may be correlated through their shared dependence on 'Other' prey items: 
...
Even if the (log) probabilities are uncorrelated between all pairs of prey items, the log-contrasts between pairs (the latent variables) are expected to be correlated. For example, if the probabilities of Pollock, Herring and Other are all uncorrelated the above equation simplifies to  
...
```{r m-tuftedb}
m.tuftedb<-MCMCglmm(cbind(Herring, Pollock, Capelin, SandLance, Other)~trait-1, data=tufted_mm, random=~idv(1+trait):year, rcov=~us(trait):units, family="multinomial", longer=20, prior=prior.tufted, pr=TRUE)
```


```{r m-tuftedc}
m.tuftedc<-MCMCglmm(cbind(Herring, Pollock, Capelin, SandLance, Other)~trait-1, data=tufted_mm, random=~idh(trait):year+year, rcov=~us(trait):units, family="multinomial", longer=20, prior=prior.tufted, pr=TRUE)
```
