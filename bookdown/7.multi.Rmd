...
Zero-altered Poisson (ZAP) models are identical to Poisson-hurdle models except the second latent variable models the probability of a non-zero (rather than a zero) and the complementary log-log link is used. The motivation behind this is that the standard Poisson can the become a special case of the zero-altered Poisson. With complementary log-log link, the probability of a zero is equal to $exp(-exp(l_2))$ where $l_2$ is the second latent variable. The probability of obtaining a zero from a Poisson distribution with mean $\lambda$ is $exp(-\lambda)$. Consequently, in a Poisson GLM with log-link the probability of a zero is $exp(-exp(l_1))$, where $l_1$ is the first latent variable. Applying the constraint that $l_1=l_2$ across all observations the zero-altered Poisson model is equivalent to the standard Poisson. In MCMCglmm, $l_1$ and $l_2$ cannot be constrained to be equal, because they must at least differ in their residual since each 'trait' must have a unique residual. Nevertheless, the residuals can be forced to have the same residual ($\texttt{unit}$) variance, although all of the information comes from the Poisson part.
...
```{r label=mzap1, echo=FALSE, include=TRUE, fig.cap="MCMC traces for the first four fixed effects from the hurdle Poisson model `m.hup` in which trait 1 ($\\texttt{art}$) is the Poisson process and trait 2 ($\\texttt{zi.art}$) is the probability of being zero.", fig.width=7, fig.height=8}
...
We can then test for zero-modification by setting up the contrasts so that the zero-altered regression coefficients are expressed as differences from the Poisson regression coefficients. When this difference is zero the variable causes no zero-modification, when it is negative it causes zero-inflation and when it is positive it causes zero-deflation:

```{r echo=TRUE, cache=TRUE}
m.zap2<-MCMCglmm(art~trait*(fem +mar + kid5 + phd + ment), rcov=~trait:units, data=bioChemists, family="zapoisson")
summary(m.zap2)
```
