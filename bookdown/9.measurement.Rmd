...
Sometimes, variables in the model are not perfectly observed. In the worst case they are completely missing, but often we have some information about them. We may only know the interval in which an observation lies, in which case the observation is said to be censored. In other cases, we have a 'best' estimate of the observation, but this estimate comes with error. Handling incomplete information about an observation is relatively straightforward when the observation is the response variable, but considerably more challenging when the observation is a predictor.   
...
It is always a little annoying when standardised values are published without the accompanying raw values or the values used to standardise. Particularly when the case for standardisation seems to be misplaced: I bet coaching companies don't have statements on their websites saying they raise SAT grades by $x$ standard deviations - they will say they raise them by $x$ points (a raw difference) or by $x$ percent (a mean standardised difference). If raw differe
