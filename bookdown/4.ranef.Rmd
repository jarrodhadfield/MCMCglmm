...
For the inverse-Wishart prior we specified the parameters $\texttt{V}$ and $\texttt{nu}$. The parameters $\texttt{alpha.mu}$ and $\texttt{alpha.V}$ can also be specified in the prior, and if $\texttt{alpha.V}$ is non-zero then parameter expansion is used. These additional parameters specify a prior for the variance, $\sigma^2_{\texttt{school}}$, that is a non-central scaled F-distribution with the numerator degrees-of-freedom set to one. The scale is set by $\texttt{alpha.V}*\texttt{V}$ and the normalised variance $\sigma^2_{\texttt{school}}/(\texttt{alpha.V}*\texttt{V})$ follows a non-central F distribution with $\texttt{df1}=1$,  $\texttt{df2}=\texttt{nu}$ and  $\texttt{ncp}=\texttt{alpha.mu}^2/\texttt{alpha.V}$. The density of $\sigma^2_{\texttt{school}}$ is therefore the density of the $F$ multiplied by $\texttt{alpha.V}*\texttt{V}$ (Section \@ref(transform-sec)). 

We can also think about this prior in terms of the standard deviation, $\sigma_{\texttt{school}}$, which in some ways is more natural, since it is in the same units as the response.  The prior distribution for the standard deviation is a scaled non-central half-$t$ (the negative values of the $t$ are ignored) with $\texttt{nu}$ degrees of freedom, non-centrality parameter $\texttt{alpha.mu}/\sqrt{\texttt{alpha.V}}$ and scale $\sqrt{\texttt{alpha.V}*\texttt{V}}$. The density of $\sigma_{\texttt{school}}$ is therefore the density of the $t$ multiplied by $\sqrt{\texttt{alpha.V}*\texttt{V}}$.
...
qqplot(pxm$VCV[,"fac"]/scale, vf)

abline(0,1)

h<-hist(pxm$VCV[,"fac"], breaks=100, prob=TRUE)
dv<-df(h$mids/scale, df1=1, df2=nu, ncp=(alpha.mu^2)/alpha.V)*scale
c<-max(h$density)/max(dv)
lines((c*dv)~h$mids, col="red")


par(mfrow=c(1,2))
sdt<-rt(2000, df=nu, ncp=alpha.mu/sqrt(alpha.V))
qqplot(sqrt(pxm$VCV[,"fac"])/sqrt(scale), sdt)

abline(0,1)

h<-hist(sqrt(pxm$VCV[,"fac"]), breaks=100, prob=TRUE)
dsd<-dt(h$mids/sqrt(scale), df=nu, ncp=alpha.mu/sqrt(alpha.V))*sqrt(scale)
c<-max(h$density)/max(dsd)
lines((c*dsd)~h$mids, col="red")
```

Before discussing the properties of $F$ and half-$t$ priors on the posterior distributions of variances and standard deviation, respectively, let's confirm that parameter expansion does indeed increase efficiency independent of the prior being used. Since these distributions only have three effective parameters we will specify $\texttt{V}=1$ throughout, without loss of generality. With $\texttt{nu=1}$ and a non-zero $\texttt{alpha.V}$ the prior on the standard deviation is a scaled half-$t$ distribution with one degree-of-freedom: a half-Cauchy distribution. If we set $\texttt{alpha.mu}=0$ we have a central half-Cauchy distribution and as we up the scale ($\sqrt{\texttt{alpha.V}}$) of this distribution it tends to a flat prior for the standard deviation. In Section \@ref(Vprior-sec) was also that an improper inverse-Wishart distribution with $\texttt{V}=0$ and $\texttt{nu}=-1$ is also flat for the standard deviation. Let's fit the improper inverse-Wishart and the central half-Cauchy with a scale that is large (relative to the data) - $\sqrt{\texttt{alpha.V}}=1000$.
...
The default in brms is to use a half-t with 3 degrees of freedom and scale of 2.5 for the standard deviation. This is less 'tailed' than the Cauchy:length
...
``` {r prior-compare, echo=FALSE, include=TRUE, fig.cap="Kernel density plots for different priors", fig.width=7, fig.height=5}
...
plot(density(samples[[1]]),                      # first density sets up the axes
     main = "Overlaid kernel densities",
     xlab = "value", ylab = "density",
     col  = cols[1], lwd = 2)

for (i in 2:length(samples))                    # add the rest
  lines(density(samples[[i]]), col = cols[i], lwd = 2)

legend("topright", lwd = 2, col = cols,
       legend = paste0("sample ", 1:6), bty = "n")
```
