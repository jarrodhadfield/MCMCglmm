...
prior.mcs.3=list(B=list(mu=rep(0,2), V=gelman.prior(~type, scale=g*2.5, intercept=g*10, data=data.cs)), 
                 R=list(V=1, fix=1))
...
Remarkably, @vanDyk.2001 show that leaving a diffuse prior on
...
For these types of problems, I usually remove the global intercept (`-1`) and use the prior $N(0, \sigma^{2}_{\texttt{units}}+\pi^2/3)$ because this is reasonably flat on the probability scale when a logit link is used. 

The probability of success in treatment 2 is consistent with the exact binomial test for which the 95% CI were (xxxx). With such a simple model, the prediction for observation 26 is equal to the treatment 2 effect and so we can get the the credible interval (on the data scale) for treatment 2 using the predict function:
