...
mbinom.2<-MCMCglmm(majority~type+ypub, family="categorical", data=Grumpy, prior=prior.mbinom.2)
```
...
mbinom.3<-MCMCglmm(majority~type+ypub, family="categorical", data=Grumpy, prior=prior.mbinom.3)
```
...
mbinom.3.plot<-MCMCglmm(majority~type+ypub, family="categorical", data=Grumpy, prior=prior.mbinom.3, nitt=13000*10)
...
The median probability of an individual bee being infected is low: $\texttt{plogis}(eta)=$ `r round(mean(plogis(mnzbinom$Sol)))` such that the expected probability of at least one infection is small for the lowest pool-size ($\texttt{Number}=$ `r min(ABPvirus$Number)`) and is still well below one in the largest pool size ($\texttt{Number}$ `r max(ABPvirus$Number)`) - see Figure \@ref(fig:nzbinom). 

```{r nzbinom, echo=FALSE, include=TRUE, fig.width=7, fig.height=5, fig.cap=paste0("Probability of seeing at least one infection in pools of varying size when the probability of an individual being infected is constant. The solid black line is the posterior mean and the shaded area is the 95\\% credible interval. The estimated median probability of an individual bee being infected is", round(mean(plogis(mnzbinom$Sol)), 3),  "and the mean probability is", round(pnzbinom[1], 3), ".")}
...
In an earlier email I said that overdispersion (between pool variation in the probability) could not be estimated as in standard binary models. If the pool size is constant this is true: the model is no different from a binary model. However, if pool size varies then there is some (little) information. To see where the information comes from first imagine that you have information on a large number of pooled samples all with a pool-size of 1. You know E[p] with certainty, where p is the probability of success for a sample within a pool. You can then move to pool sizes of 2. If there is no variation in p, P (the probability of one success in the pool) is 1-(1-p)^2. However, because this is a non-linear function of p it is not true that E[1-(1-p)^2]=1-(1-E[p])^2 when there is variation in p. Jensen's inequality tells us that E[f(x)]<f(E[x]) when f(x) is convex, which 1-(1-p)^2 is: we expect the the number of successful pools of size 2 to be less than that predicted from estimating p from pools of size 1 if between-pool variation in p exists. This also implies that estimates from large pools underestimate p when there is between pool variation in p.
