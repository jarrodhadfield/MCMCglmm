...
However, for this model we have to add the intercept to both random effects to get the year predictions. `MCMCglmm` does not store the posterior distribution of the random effects by default, but because we specified `pr=TRUE`, the whole of $\boldsymbol{\theta}$ is stored rather than just ${\boldsymbol{\mathbf{\beta}}}$:
