...
The inverse-logit function is the cumulative distribution function for the logistic distribution. It makes sense that a cumulative distribution function for a continuous distribution that can take any value would serve as a good inverse link function for a probability: it takes any value between plus and minus infinity and squeezes it to be between zero and one. While the logit is the most common link function for binomial data, the probit link and complementary log-log link are also widely used. They are the cumulative distribution functions for the unit normal and extreme value distributions respectively. In Figure \@ref(fig:link) we can see how the probability changes as a function of the input ($l$) for the different links. 


```{r link, echo=TRUE, include=TRUE, fig.width=7, fig.height=5, fig.cap=""}

pos.l<-seq(-3, 3, length=100)

plot(plogis(pos.l)~pos.l, type="l")
lines(pnorm(pos.l)~pos.l, col="red")
lines(1-exp(-exp(pos.l))~pos.l, col="blue")
```
