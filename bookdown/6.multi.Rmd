...
A simple regression suggests the two types of spending are negatively related but the association is weak with the $R^{2}=$`r round(summary(lm(y.car~y.hol))$r.squared,3)`.
...
m5a.2<-MCMCglmm(cbind(y.hol, y.car)~trait-1, random=~us(trait):id, rcov=~us(trait):units, data=Spending, family=c("gaussian", "gaussian"))
```
...
As is often the case the parameters of the zero-inflation model mixes poorly (See Figure \@ref(fig:ZIP) especially when compared to equivalent hurdle models (See Section \@ref(Hurdle)). Poor mixing is often associated with distributions that may *not* be zero-inflated but instead over-dispersed.
...
Hurdle models are very similar to zero-inflated models but they can be used to model zero-deflation as well as zero-inflation and seem to have much better mixing properties in `MCMCglmm`. As in ZIP models each datum
