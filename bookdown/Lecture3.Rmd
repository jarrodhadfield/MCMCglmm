...
vignette: |-
  ...
  %\VignetteIndexEntry{Lecture3-knitr.Rnw}
  %\VignetteDepends{}
output:
...
Random effect specification is a common cause of confusion, especially
when we want to form interactions in the random terms. To illustrate the
possibilities we'll use data collected on Blue tits.

``` {r echo=TRUE, cache=FALSE}
data(BTdata)
```

The data are morphological measurements (`tarsus` length and `back`
colour) made on 828 blue tit chicks from 106 mothers (`dam`). Half the
...
recorded as `fosternest`.

``` {r echo=TRUE, cache=TRUE}
prior=list(R=list(V=1, nu=0.002), G=list(G1=list(V=1, nu=0.002),G2=list(V=1, nu=0.002)))
m3a.1<-MCMCglmm(tarsus~sex, random=~dam+fosternest, data=BTdata, verbose=FALSE, prior=prior)
```

fits `sex` as a fixed effect, and `dam` and `fosternest` as random
effects.
...
Perhaps the autocorrelation for the `fosternest` variance is a little
higher than we would like, and so we may like to run it for longer.

``` {r echo=TRUE}
effectiveSize(m3a.1$VCV)
```

Indeed, we've only sampled the fosternest variance about half as well as
the other two variance components.\
...
which is not that surprising give the data come from an experiment which
...
-1 as the prior information goes to zero.\
...
and perhaps a model without it would be better supported, although the
DIC suggest not:

``` {r echo=TRUE, cache=TRUE}
priorb<-prior
priorb[[2]]<-priorb[[2]][-2]
m3a.2<-MCMCglmm(tarsus~sex, random=~dam, data=BTdata, verbose=FALSE, prior=priorb)
m3a.2$DIC
m3a.1$DIC
```

The tarsus lengths were standardised prior to analysis - this is not
recommended, but was done in the original analyses of these data
[@Hadfield.2007] so that comparisons would be scale invariant. The
...
proportion of the total variance is explained by dams

``` {r echo=TRUE}
HPDinterval(m3a.1$VCV[,1]/rowSums(m3a.1$VCV))
```

One nice thing though about standardised data is that effect sizes are
immediately apparent. For example, fixed effects are in standard
deviation units and the sex effects are non-trivial:

``` {r echo=TRUE}
summary(m3a.1)
```

Given that the sexes differ in their mean phenotype it may be worth
...
necessary to understand how the variance functions, such as `us()` and
`idh()`, work. We could refit the model `m3a.1` using the random effect
specifications:
...
$^{\sim}$`dam+fosternest`. The term inside the brackets is a model
...
interaction for the first few levels of `dam`:
...
Across the top, we have the original dam effects in red, and along the
...
Although they have different names from the original `dam` levels, it is
...
complex interactions this is not the case.\
We could also fit `sex` in the variance structure model, (i.e.
`us(``sex``):``dam` or `idh(``sex``):``dam`)[^1]:
...
which creates three times as many random factors, one associated with
offspring of each sex for each each dam.

## `idh` Variance Structure

The different variance functions make different assumptions about how
...
chick. We can fit this model using the `idh` function and represent our
belief in how the effects are distributed as a $3\times3$ covariance
matrix ${\bf V}$:
...
[\[chap1\]](#chap1){reference-type="ref" reference="chap1"} and
[\[chap2\]](#chap2){reference-type="ref" reference="chap2"} ${\bf V}$
was a scalar (${\bf V} = \sigma^{2}$) rather than a matrix, and the
prior specification was relatively simple. We will come back to prior
specifications for covariance matrices in Section
[6](#VCVprior-sec){reference-type="ref" reference="VCVprior-sec"}, but
for now note that the prior for the dam component has `V` as a
$3\times3$ identity matrix:

``` {r echo=FALSE, eval=TRUE, cache=TRUE}
priorb=list(R=list(V=diag(1), nu=0.002), G=list(G1=list(V=diag(3), nu=0.002),G2=list(V=1, nu=0.002)))
m3a.3<-MCMCglmm(tarsus~sex, random=~idh(sex):dam+fosternest, data=BTdata, verbose=FALSE, prior=priorb)
```
...
``` {r label=BTidh, echo=FALSE, include=TRUE, fig.cap="MCMC summary plot for the sex-specific dam variance components from model \\texttt{m3a.3}. The number of chicks with unknown (\\texttt{UNK}) sex is low, with very little replication within dams. The posterior distribution for the \\texttt{UNK} variance component is dominated by the prior which has a marginal distribution of \\texttt{V=1} and \\texttt{nu=0.002}.", fig.width=7, fig.height=5}
plot(m3a.3$VCV[,1:3])
```

The sex specific variances for males and females look pretty similar,
...
`r sum(BTdata$sex=="UNK")` birds with unknown sex and these tend to be
...
variance in model `m3a.1`
(`r formatC(mean(m3a.1$VCV[,1]),format="f", 3)`) where a sex interaction
...
distribution (you can rotate the figure in R):

``` {r label=rgl, echo=TRUE, eval=FALSE, fig.cap="Ellipsoid that circumscribes 95\\% of the expected dam effects as estimated in model \\texttt{m3a.3}. This can be thought of as a scatter plot of the dam effects between each sex, if the dam effects could be directly measured.  Because the covariances of the dam effects between the sexes were set to zero the axes of the ellipsoids are all parallel to the figure axes."}
plotsubspace(Vdam.3, axes.lab=TRUE)
```

If we had measured the offspring of a lot of dams, and for each dam we
...
The oddity of the model, and the meaning of the off-diagonal zeros,
...
assumption using the `us` function which estimates the matrix:
...
We will now use a prior for the covariance matrix where nu=4 (1 more
than the dimension of **V**) and the prior covariance matrix is an
diagonal matrix with small variances. This may seem surprising but the
motivation is laid out in Section
[6](#VCVprior-sec){reference-type="ref" reference="VCVprior-sec"}:

``` {r echo=TRUE, cache=TRUE}
prior.m3a.4=list(R=list(V=diag(1), nu=0.002), G=list(G1=list(V=diag(3)*0.02, nu=4),G2=list(V=1, nu=0.002)))
m3a.4<-MCMCglmm(tarsus~sex, random=~us(sex):dam+fosternest, data=BTdata, verbose=FALSE, prior=prior.m3a.4)
```

The posterior mean (co)variances for this model show that the
...
All the correlations are very close to one, and the variances all pretty
...
There are also ways of specifying models that lie somewhere between the
simple model (`mBT`), where dam effects are assumed to be equivalent
across the sexes, and the most complex model (`mBT2`), where dam effects
...
[1](#tab:T1){reference-type="ref" reference="rspec"}.\
To be completed \....\
...
::: {#rspec}
  ...
  ------------------------- --------------------- ---------------- --------------------------------------------------------------------------------------------------------------- ---------------------------------------------------
   ...
   `(1|dam)`+`(1|sex:dam)`     `dam`+`sex:dam`           2                                                             $\left[                                                                           $\left[
                                                                                                                 \begin{array}{ccc}                                                                \begin{array}{ccc}
                                                                                                              V_{1}+V_{2}&V_{1}&V_{1}\\                                                           {\color{red} 1}&r&r\\
                                                                                                              V_{1}&V_{1}+V_{2}&V_{1}\\                                                           r&{\color{red} 1}&r\\
                                                                                                              V_{1}&V_{1}&V_{1}+V_{2}\\                                                           r&r&{\color{red} 1}\\
                                                                                                                     \end{array}                                                                       \end{array}
                                                                                                                      \right]$                                                                          \right]$

                                                                                                                                                                                   

             \-                `idh(sex):dam`            3                                                             $\left[                                                                           $\left[
                                                                                                                 \begin{array}{ccc}                                                                \begin{array}{ccc}
  ...
  : (#tab:T1) Different random effect specifications in `lmer`,
  `MCMCglmm` and `asreml`. `sex` is a factor with three levels so the
  resulting matrix is $3\times3$. Continuous variables can also go on
  the LHS of the pipe, or within the variance structure functions (e.g.
  `us`,`idh`). In this case the associated parameters are regression
  ...
  for chicks raised by the same mother. `(1+age|dam)` or `us(1+age):dam`
  estimates a $2\times2$ matrix which includes the variance in
  intercepts (when `age`=0), the variance in slopes, and the covariance
  that exists between them. Fixed parameters are in red.
:::
::::

## Heterogenous Residual Variance {#heter-sec}

To be started\... In short - if you've fitted a sex by dam interaction I
would always allow the sexes to have different residual variances. Use
`rcov=`$\sim$`idh(sex):units`.

## Contrasts and Covariances

A general method for seeing what a particular random specification means
in terms of the original variables is to realise that

$${\boldsymbol{\mathbf{\Sigma}}} = {\bf Z}{\bf V}{\bf Z}^{'}
\label{conv-eq}   (\#eq:conv-eq)$$

where ${\boldsymbol{\mathbf{\Sigma}}}$ is the covariance matrix for the
original set of variables and ${\bf V}$ the variances associated with
the variance structure model. ${\bf Z}$ is the random effect design
matrix. Equation \@ref(eq:conv-eq) implies:
...
if ${\bf Z}$ is non-square and/or singular, where $^{-}$ is a
generalised inverse.\

## Priors for Covariance Matrices {#VCVprior-sec}

Priors for covariance matrices are tricky. What maybe non-informative
for a covariance may be informative for a correlation and *vice versa*.

### Priors for `us` structures

A useful result is that the marginal distribution of a variance is also
inverse - Wishart distributed:

$$\sigma^{2}_{1} \sim IW\left(\texttt{nu}^{\ast}\texttt{=nu-dim(V)+1},\ \texttt{V}^{\ast}=\frac{\texttt{nu}}{\texttt{nu}^{\ast}}\texttt{V[1,1]}\right)$$

using the first variance as an example, and indicating the new
parameters with an asterisk.\
An uninformative prior for the correlations is an improper prior with
`V=diag(dim(V))`$\ast$`0` and `nu=dim(V)+1`. For the $3\times3$ sex by
dam covariance matrix in model `m3a.4` we used a proper prior with
`V=diag(3)`$\ast$`0.02` and `nu=4` in the hope that this would be
...
[\[chap1\]](#chap1){reference-type="ref" reference="chap1"}:
...
dv<-dgamma(1/xv, shape=nu.ast/2, scale=(nu.ast*V.ast)/2)/(xv^2)
plot(dv~xv, type="l")
```

In Chapter [\[chap2\]](#chap2){reference-type="ref" reference="chap2"}
we saw that a non-informative prior for a variance component was `V=0`
and `nu=-2`. This result generalises to covariance matrices where the
improper prior `V=diag(dim(V))`$\ast$`0` and `nu=dim(V)-3` is
...
For `idh` the diagonal elements of the matrix are independent and each
...
For `corg` and `corgh` structures[^3] the diagonals of `V` define the
fixed variances (`corgh`) or are ignored and the variances set to one
(`corg`). I use the prior specification in @Barnard.2000 where `nu`
controls how much the correlation matrix approaches an identity matrix.
The marginal distribution of individual correlations ($r$) is given by
...
and as shown above setting `nu` =`dim(V)+1` results in marginal
correlations that are uniform on the interval \[-1,1\].\
In most cases correlation matrices do not have known form and so cannot
be directly Gibbs sampled. `MCMCglmm` uses a method proposed by
@Liu.2006 with the target prior as in @Barnard.2000. Generally this
...
is equivalent to the prior in @Barnard.2000 with `nu=0` so as long as a
diffuse prior is set, mixing is generally not a problem. If `nu=0` is
...
[]{#VCVprior-sec label="VCVprior-sec"}
:::::

[^1]: Remember that a global intercept is not fitted by default for
    variance structure models, and the model formula is essentially
    $^{\sim}$`sex-1`. To add the global intercept, `us(``1+sex``):``dam`
    could be fitted but this can be harder to interpret because the
    effects are then `Fem`, `Male-Fem` and `UNK-Fem`. If a `us`
    structure is fitted, the two models are equivalent
    reparameterisations of each other although the priors have to be
    modified accordingly. This is not the case if the variance function
    is `idh`. In this case the sex-specific variances are allowed to
    vary as before, but a constant covariance equal to
    $\sigma^{2}_{\color{blue} \texttt{Fem}}$ is also assumed

[^2]: IMPORTANT: In versions $<$ 2.05 priors on each variance of an
    `idh` structure were distributed as
    $IW\left(\texttt{nu}^{\ast}\texttt{=nu-dim(V)+1},\ \texttt{V}^{\ast}=\texttt{V[1,1]}\right)$
    but this was a source of confusion and was changed.

[^3]: In versions $<2.18$ `cor` fitted what is now a `corg` structure.
    The reason for the change is to keep the `asreml` and `MCMCglmm`
    syntax equivalent. However, the `corgh` structure in asreml is a
    reparameterised `us` structure whereas in `MCMCglmm` the variances
    are fixed in the prior.
