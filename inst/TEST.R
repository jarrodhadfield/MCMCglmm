...
for(i in 1:nsim){
tree<-rcoal(1000)
y<-rbv(tree, diag(1), nodes="TIPS")+rnorm(1000)
A<-inverseA(tree)
...
for(i in 1:nsim){
 ...
 m1<-MCMCglmm(y~trait-1, rcov=~us(at.level(trait,1)):units+idh(at.level(trait,3)):units+idh(at.level(trait,2)):units, data=dat, family=NULL, prior=prior, verbose=verbose, nitt=nitt, thin=thin, burnin=burnin)
...
for(i in 1:nsim){
 ...
 m1<-MCMCglmm(cbind(y1, y2, y3)~trait-1+at.level(trait,1):fac+path(1,2,3)+path(2,3,3), rcov=~idh(trait):units, data=dat, family=rep("gaussian", 3), verbose=verbose, nitt=nitt, thin=thin, burnin=burnin)
...
for(i in 1:nsim){
  ...
  prior<-list(R=list(R1=list(V=V, nu=3, covu=TRUE, fix=2), R2=list(V=Vr, nu=1)))
   ...
   res55[i,]<-c(posterior.mode(m1$Sol), posterior.mode(m1$VCV))
